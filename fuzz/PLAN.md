...
## 0. Shadowing / name-reuse mutation  ✰ proven-miss, DONE

`fuzz_mutate` now has a mutation that renames a named `let` to an
identifier its own initializer reads (`get_names_in` + the `Let` arm of
`mutate_node`). The wax corpus is decompiled from Wasm, so every name is
unique and *no* seed ever shadows — the source-level scoping/renaming paths
in `to_wasm.ml` were therefore dark. This manufactures the missing shape: a
`let` that shadows an outer binding (a parameter or enclosing local) whose
initializer references it. It caught a live To_wasm miscompile (the
initializer bound to the new, renamed local instead of the outer one).

- Oracle pairing: the **type-changing** variant (a ref-typed initializer
  whose condition reads an `i32` param, renamed onto that param) surfaces as
  a `FALSE_ACCEPT` under the existing `oracle.sh` (wax accepts it, wasm-tools
  rejects the emitted module). The **same-type** variant is a *silent* value
  miscompile that still validates — it needs a differential-execution oracle
  (`exec-ref.sh`), so this mutation only pays off fully once item 5 wires the
  execution oracles into the mutation campaigns.
...
## 10. Post-mortem oracles from the 2026-07 validation fixes — LANDED

*Each targets the exact gap that let one of the four fixed bugs slip every
campaign; each is verified to bite (turning its fix off makes it go red).*

- [x] **Cross-proposal corpus + mutator** (`cross-corpus.sh` +
      `wast-extract.js`, wired into `build-corpus.sh`; `mode=cross` of
      `wat-type-mutate.awk` + `wat-cross-proposal.sh`) — the corpus had zero
      `exact`/`cont`/`switch` constructs (wasm-smith knows neither proposal),
      so the `switch`-on-`(ref (exact $cont))` `assert false` (exit 125) was
      unreachable to every campaign. Now the in-tree proposal suites are
      harvested as self-declaring text seeds and the mutator grafts
      exact/cont/descriptor constructs across proposals.
- [x] **Unreachable-insertion metamorphic oracle** (`unreachable-fuzz.sh` +
      `wat-unreachable-insert.awk`) — validity-preserving `unreachable`
      insertion, REF-arbitrated; catches the `extern.convert_any` /
      `any.convert_extern` over-rejection on a polymorphic stack (`Bot` was
      typed nullable, not the principal non-null).
- [x] **No-duplicate-diagnostics invariant** (oracle 2b, `DIAG_DUP`) — a
      rejection must not repeat a located diagnostic line; the class the
      validator's `muted` pre-pass de-duplication fixed. Residual classes found
      on the current tree are recorded as REVIEW findings (br_table per-target
      repeats, doubled if/select stack-shape errors) — see the corpus run.
- [x] **Single-fault locality oracle** (`fault-locality.sh` +
      `wat-fault-mutate.js`) — one retargeted use-site reference must yield
      exactly one local error; the regression guard for poisoned index entries
      (pre-poisoning, a failed definition shifted every later index and errors
      cascaded module-wide).
- [x] **Wax typer mirror of the above** (2026-07-18/19, see
      `TYPING-FINDINGS-TRIAGE.md`): `wax-fault-mutate.js` + `.wax` bases in
      `fault-locality.sh` (callee / label / construction-name faults), and the
      `DIAG_DUP` invariant on `type-fuzz.sh` rejects. Surfaced and fixed the
      typer's same-location duplicate classes, the unbound-construction-name
      silent accept ([lookup_*_type] now reports), the signature-failure
      cascade (functions poison-registered, bodies still checked), and the
      recovery-arity desyncs (a [Poisoned] stack state + branch-recovery
      pass-through); all mutator exclusions lifted except the explicit-type
      signature form.
