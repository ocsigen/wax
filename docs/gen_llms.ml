(* Generate a single-file, context-loadable Wax reference by concatenating the
   mdbook sources in reading order. The output is committed as the `wax` agent
   skill's reference.md (and embedded into the `wax mcp` binary).
