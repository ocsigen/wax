...
let strip_constructor s =
  ...
  then
    ...
    match split_on_char ',' content with _ :: x :: _ -> trim x | _ -> s
    (* The project's trailing-comma list variants (see [separated_*_trailing] in
     the grammars). Menhir names the instantiation with the separator token
     ([COMMA]), so without this the whole [separated_nonempty_list_trailing(...)]
     name leaked verbatim into messages. Strip to the element, as above. *)
  else if
    starts_with ~prefix:"separated_list_trailing(" s && ends_with ~suffix:")" s
  then
    let content = extract_content (length "separated_list_trailing(") 1 in
    match split_on_char ',' content with _ :: x :: _ -> trim x | _ -> s
  else if
    starts_with ~prefix:"separated_nonempty_list_trailing(" s
    && ends_with ~suffix:")" s
  then
    let content =
      extract_content (length "separated_nonempty_list_trailing(") 1
    in
    match split_on_char ',' content with _ :: x :: _ -> trim x | _ -> s
  else s

(* Whether a symbol is one of Menhir's list constructors. In the "Expecting …"
   path these are stripped to their element by [expand_symbol]; this is for the
   "Assuming that the … is complete" path, which names the reduced nonterminal
   directly — there [nonempty_list(f64)] would otherwise leak verbatim, so it is
   named generically "a list". *)
let is_list_symbol s =
...
let get_readable_name terminals s =
  ...
  with Not_found -> (
    ...
    (* WAT annotation openers; unaliased tokens otherwise print as
       '(then)'-less jargon like 'then_annot'. *)
    | "IF_ANNOT" -> "'(@if'"
    | "THEN_ANNOT" -> "'(@then'"
    | "ELSE_ANNOT" -> "'(@else'"
    ...
    (* WAT (lib-wasm) nonterminals whose derived name is an abbreviation or reads
       oddly. *)
    | "result_pat" -> "a result pattern"
    | "float_or_nan" -> "a float or NaN"
    (* Head noun is "list", not the plural tail: keep the article so the
       "… is complete" template agrees ("the list of indices is", not "are"). *)
    | "list_of_indices" -> "a list of indices"
    | _ ->
        if is_list_symbol s then "a list"
        else if String.lowercase_ascii s = s then
