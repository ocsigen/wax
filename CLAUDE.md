...
|      | `--desugar` | Expand the Wax-specific `(@string …)`/`(@char …)` annotations into core wasm (`array.new_fixed`/`i32.const`) and strip `(@feature …)` (pure metadata; resolution already ran) so the output is plain WebAssembly text with no annotation left. Wat output only (usage error `123` otherwise); fails (`128`) if an `(@if …)` remains unresolved — resolve with `-D` |
