...
| `-W` | `--warn` | Set a warning's level: `NAME=LEVEL` where `NAME` is a warning (`unused-local`, `unused-field`, `unused-label`, `shift-count-overflow`, `constant-trap`, `tautological-comparison`, `constant-condition`, `unused-result`, `dead-code`, `truncated-coverage`, `naming-conflict`, `reserved-word-rename`, `generated-name`), a group (`unused`, `correctness`, `naming`), or `all`, and `LEVEL` is `hidden`/`warning`/`error`. The `correctness` lints run during validation and are shown by default; the `naming` warnings (Wasm→Wax renames/generated names) are hidden by default. The correctness lints live in the Wax typer (`lib-wax/typing.ml`); the constant-operand and dead-code ones are mirrored in the Wasm validator (`lib-wasm/validation.ml`, `lint_body`) so they also fire on WAT/WASM input. `unused-field` and `unused-label` are mirrored too (also in `validation.ml`): `unused-label` via a usage flag on each control frame set in `branch_target`, `unused-field` via marking resolved function/global indices used in `get_function`/`get_global` plus exports/start; both are gated by `warn_unused` like `unused-local`. Later settings override earlier; repeatable. The `WAX_WARN` env var (comma/space-separated `NAME=LEVEL` specs) seeds defaults applied *before* `-W` — it layers under `-W` (unlike cmdliner's built-in env fallback), so `-W` refines it rather than replacing it. |
