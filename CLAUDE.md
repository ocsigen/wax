...
|      | `--debug` | Enable developer debug output for a category (repeatable, comma-separated). Categories: `timing` (log each pass's wall-clock time to stderr), `width-check` (on wasm/wat→wax, the differential width check: `from_wasm` records the type each source opcode states on the Wax node it emits — `Ast.instr`'s `expected` field, set by `expect`/`cast_to`/`typed_hole`/`Stack.push_num`/`pin_width` and cleared by `forget_expected` where the width comes from context — and `Typing.f ~width_check` compares its own *resolved* type for the node (`project_annotation`'s defaults, so a flexible literal is compared post-defaulting) against it, reporting a decompiler width drift as an error (exit 128) rather than printing Wax that recompiles at another width) |
