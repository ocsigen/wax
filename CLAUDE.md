...
|      | `--faithful` | Decompile without the stream-reshaping recoveries, so a wat/wasm module decompiled to wax recompiles with the same reachable instruction structure. Wax output only (usage error `123` otherwise). Reliably gates two recoveries: `t.eq; i32.eqz` stays `!(a==b)` (gated in `from_wasm.ml`'s `int_un_op`) instead of fusing to `t.ne`; and `Recover_match`'s flat `br_on_cast_fail`-chain arm (not the nested-ladder arm, an exact inverse that stays) is skipped. The typer's simplify pass is off (`Typing.f ~simplify:(not faithful) ~faithful`) so a redundant *non-null* `ref.cast` is kept and re-emits; `ctx.faithful` still prunes the nullable type-pin scaffolding casts (`unnecessary_cast`, since compiler pins and source casts are indistinguishable without provenance — cast fidelity is best-effort). `from_wasm` under faithful also pins constant/float widths through int→float conversions, `i64.extend32_s`, and `f32` methods so they do not re-default. Threaded `main.ml` (`wat_to_wax`/`wasm_to_wax`) → `From_wasm.module_ ~faithful` → `Recover_match.module_ ~faithful` + `Typing.f ~faithful`. Ungateable residuals (all inert, present on the default path too): `to_wasm` recompiler-peephole fusions (narrow-load-then-widen `i32.load8_s; i64.extend_i32_s` → `i64.load8_s`; `extend32_s`; direct-call), local/name/type renumbering, and dead code. Proven by `fuzz/oracle.sh`'s `FAITHDRIFT` leg: a reachable-only opcode sequence with widths, the compiler-cast family and the fusions normalised away (documented there) |
