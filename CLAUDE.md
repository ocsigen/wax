...
|      | `--debug` | Enable developer debug output for a category (repeatable, comma-separated). Categories: `timing` (log each pass's wall-clock time to stderr), `width-check` (on wasm/wat→wax, **report** the differential width check instead of **repairing** it, which is the default). The channel: `from_wasm` records the type each source opcode states on the Wax node it emits — `Ast.instr`'s `expected` field, set by `expect`/`cast_to`/`typed_hole`/`Stack.push_num`/`pin_width` and cleared by `forget_expected` where the width comes from context — and `Typing.f ~width_check:[`Off|`Repair|`Report]` (`reconcile_widths`, after typing and simplify, before `project_annotation`) compares its own *resolved* type for the node (the projection's defaults, so a flexible literal is compared post-defaulting) against it. Under `` `Repair ``, the decompile default, a value whose width merely DEFAULTED wrong is wrapped in the identity cast that pins it (and its inference cell is grounded with `Cell.set`, so the unified cells of its tree settle too and one pass converges) — a missing `from_wasm` pin becomes an extra cast, not a silent miscompile; `type_cast`'s `operand_pin_pending` keeps a cast whose operand is awaiting such a pin, so the pin has somewhere to attach. Under `` `Report `` (this flag) the same disagreement is an error (exit 128) instead, which is what keeps the fuzz harness able to see a missing pin rather than a healed one (`fuzz/oracle.sh` oracle 5c, `fuzz/drop-width.sh`). A disagreement no pin can fix — the value's type is fixed by context (`pin_stays_in_family`/`flexible_literal`/`defaulting_tree` decide), so a cast would convert rather than ground it — is an error in BOTH modes: either the input is invalid (a binary is trusted, never validated) or the conversion is wrong) |
