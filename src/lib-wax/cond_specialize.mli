(** Specialization of Wax conditional annotations ([#[if(...)]] / [#[else]])
    ...
    {!Wax_wasm.Cond_specialize.module_}. *)

val module_ :
  Wax_utils.Diagnostic.context ->
  Wax_wasm.Cond_specialize.bindings ->
...
(** Splice out and simplify every conditional annotation in a Wax module,
    according to {!Wax_wasm.Cond_specialize.eval}. Returns the specialized
    module and the half-open byte ranges of the branches that were removed, for
    dropping their comments (see {!Wax_utils.Trivia.drop_in_ranges}). With empty
