...
(** As {!f}, but returns the typed tree with its inference cells intact rather
    ...
    appends the field's span and a {!Members.member_receiver} describing what it
    is on, from which {!Members.member_candidates} derives the completion list
    on demand. All default to [None], so an ordinary run records nothing. *)
