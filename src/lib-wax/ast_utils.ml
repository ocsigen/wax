...
(* Whether any branch within [i] targets the label [name]. A conservative
   over-approximation: a nested block that shadows [name] is not accounted for,
   so a branch to such a shadow is still counted — which only ever keeps a block
   result annotation that might in principle be dropped, never anything unsound.
   Used to leave the [=> T] annotation on a value block reached by a [br]. *)
let rec refs_label name (i : _ instr) : bool =
  let any = List.exists (refs_label name) in
  let opt = function Some x -> refs_label name x | None -> false in
  let lab (l : label) = String.equal l.desc name in
  match i.desc with
  | Br (l, e) -> lab l || opt e
  | Br_if (l, e) -> lab l || refs_label name e
  | Br_table (ls, e) -> List.exists lab ls || refs_label name e
  | Br_on_null (l, e) | Br_on_non_null (l, e) -> lab l || refs_label name e
  | Br_on_cast (l, _, e) | Br_on_cast_fail (l, _, e) ->
      lab l || refs_label name e
  | Dispatch { index; cases; default; arms } ->
      lab default || List.exists lab cases || refs_label name index
      || List.exists (fun (_, b) -> any b) arms
  | Match { scrutinee; arms; default } ->
      refs_label name scrutinee
      || List.exists (fun (_, b) -> any b) arms
      || any default
  | Block { block; _ } | Loop { block; _ } | TryTable { block; _ } -> any block
  | While { cond; block; _ } -> refs_label name cond || any block
  | If { cond; if_block; else_block; _ } -> (
      refs_label name cond || any if_block.desc
  ...
  | ContNew (_, e) ->
      refs_label name e
  | Call (a, l) | TailCall (a, l) -> refs_label name a || any l
  | Struct (_, fs) -> List.exists (fun (_, e) -> refs_label name e) fs
  | StructSet (a, _, b)
  | Array (_, a, b)
  | ArraySegment (_, _, a, b)
  | ArrayGet (a, b)
  | BinOp (_, a, b) ->
      refs_label name a || refs_label name b
  | ArraySet (a, b, c) | Select (a, b, c) ->
      refs_label name a || refs_label name b || refs_label name c
  | ArrayFixed (_, l)
  | ContBind (_, _, l)
  | Suspend (_, l)
  | Resume (_, _, l)
...
let refs_label_list name l = List.exists (refs_label name) l
