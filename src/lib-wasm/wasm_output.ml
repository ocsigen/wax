...
let module_ ~out_channel ?output_file ?(source_map = false)
  ...
  (* The DWARF line table is built from the very mappings a source map is made
     of, so [-g] needs the sink recording even when no [.map] file is asked for.
     Only the serialization below stays gated on [source_map]. *)
  let source_map_t =
    Wax_utils.Source_map.create ~enabled:(source_map || dwarf)
  in
