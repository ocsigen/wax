...
let check_all diagnostics ?truncation_location
  ...
  while (not (Queue.is_empty queue)) && not !truncated do
    ...
    if Bdd_tbl.mem processed asm || not (Cond_solver.is_satisfiable asm) then ()
      ...
      check cctx cfg;
      configurations := (Diagnostic.collected cctx, !a_full) :: !configurations
    end
  done;
  report diagnostics ?truncation_location ~explain:(explain env)
    ~truncated:!truncated (List.rev !configurations)
