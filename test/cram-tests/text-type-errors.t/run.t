A type-mismatch error now prints the value on the stack as the source wrote it
— naming an indexed type by its source reference ($name) rather than the
interned canonical index, which is opaque and can even differ from what the
user typed when distinct source types share a canonical index.
