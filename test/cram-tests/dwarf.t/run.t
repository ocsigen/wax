...
Validate DWARF structures via llvm-dwarfdump (if available):
  ...
  > else
  >   # Mock what the greps above select, so the test is deterministic on a
  >   # machine without llvm-dwarfdump (CI installs none).
  >   echo "Line table prologue:"
