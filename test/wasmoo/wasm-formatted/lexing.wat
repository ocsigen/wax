...
(func (export "caml_lex_engine")
  ...
  (local $state i32) (local $buffer (ref $bytes)) (local $vpos (ref eq))
  (local $action (ref eq)) (local $pos i32) (local $base i32)
  (local $backtrk i32) (local $lex_base (ref $bytes))
  (local $lex_backtrk (ref $bytes)) (local $lex_check (ref $bytes))
  (local $lex_trans (ref $bytes)) (local $lex_default (ref $bytes))
...
(func (export "caml_new_lex_engine")
  ...
  (param $vlexbuf (ref eq)) (result (ref eq))
  (local $tbl (ref $block)) (local $lexbuf (ref $block)) (local $c i32)
  (local $state i32) (local $pstate i32) (local $buffer (ref $bytes))
  (local $vpos (ref eq)) (local $action (ref eq)) (local $pos i32)
  (local $base i32) (local $backtrk i32) (local $pc_off i32)
  (local $base_code i32) (local $lex_code (ref $bytes))
  (local $lex_base (ref $bytes)) (local $lex_base_code (ref $bytes))
  (local $lex_backtrk (ref $bytes)) (local $lex_backtrk_code (ref $bytes))
  (local $lex_check (ref $bytes)) (local $lex_check_code (ref $bytes))
  (local $lex_trans (ref $bytes)) (local $lex_trans_code (ref $bytes))
  (local $lex_default (ref $bytes)) (local $lex_default_code (ref $bytes))
  (local.set $tbl (ref.cast (ref $block) (local.get $vtbl)))
  (local.set $lexbuf (ref.cast (ref $block) (local.get $vlexbuf)))
  (local.set $state (i31.get_s (ref.cast (ref i31) (local.get $start_state))))
  (local.set $buffer
