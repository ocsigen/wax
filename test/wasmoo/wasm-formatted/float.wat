...
(@if $wasi
(@then (import "io" "IO_BUFFER_SIZE" (global $IO_BUFFER_SIZE i32))
...
(global $infinity (ref $chars)
  (array.new_fixed $chars 8 (@char "i" ) (@char "n" ) (@char "f" )
    (@char "i" ) (@char "n" ) (@char "i" ) (@char "t" ) (@char "y" ))
)
...
(func (export "caml_format_float")
  (param $vfmt (ref eq)) (param $arg (ref eq)) (result (ref eq))
  (local $f f64) (local $b i64) (local $sign_style i32) (local $precision i32)
  (local $conversion i32) (local $uppercase i32) (local $negative i32)
  (local $exp i32) (local $m i64) (local $i i32) (local $len i32)
  (local $c i32) (local $s (ref $bytes)) (local $txt (ref $chars))
  (local $num anyref)
  (local.set $f
    (struct.get $float 0 (ref.cast (ref $float) (local.get $arg))))
      ...
      (then
        ...
        (if (i32.eq (i32.and (local.get $c) (i32.const 0xdf)) (@char "N" ))
          (then
            ...
            (local.set $c (array.get_u $bytes (local.get $s) (local.get $i)))
            (if
              (i32.eq (i32.and (local.get $c) (i32.const 0xdf)) (@char "A" ))
              (then
                (local.set $i (i32.add (local.get $i) (i32.const 1)))
                (local.set $c
                  (array.get_u $bytes (local.get $s) (local.get $i)))
          ...
          (else
            (if
              (i32.eq (i32.and (local.get $c) (i32.const 0xdf)) (@char "I" ))
              (then
                (local.set $i (i32.add (local.get $i) (i32.const 1)))
                (local.set $c
                  (array.get_u $bytes (local.get $s) (local.get $i)))
      ...
      (then
        ...
        (if (i32.eq (i32.and (local.get $c) (i32.const 0xdf)) (@char "I" ))
          (then
            ...
            (local.set $c (array.get_u $bytes (local.get $s) (local.get $i)))
            (if
              (i32.eq (i32.and (local.get $c) (i32.const 0xdf)) (@char "N" ))
              (then
                (local.set $i (i32.add (local.get $i) (i32.const 1)))
                (local.set $c
                  (array.get_u $bytes (local.get $s) (local.get $i)))
          ...
          (i64.add
            (select (i64.const 1) (i64.const -1)
              (i32.xor (f64.lt (local.get $y) (f64.const 0))
                (f64.gt (local.get $z) (f64.const 0)))) (local.get $4)))))
