...
(@if $wasi
  (@then
    (func (export "caml_sys_read_directory")
      ...
      (local $new_buf i32) (local $size i32) (local $pos i32)
      (local $available i32) (local $left i32) (local $namelen i32)
      (local $entry i32) (local $entry_size i32) (local $cookie i64)
      (local $tbl (ref $block)) (local $new_tbl (ref $block)) (local $i i32)
      (local $s (ref $bytes))
      (call $caml_sys_resolve_path (local.get $name))
      (local.set $p_len)
      (local.set $p_addr)
