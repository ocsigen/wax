...
(func (export "caml_floatarray_make")
  ...
  (if (i32.ge_u (local.get $sz) (i32.const 0x7ffffff))
    (then (call $caml_invalid_argument (global.get $Array_make))))
  (if (i32.eqz (local.get $sz)) (then (return (global.get $empty_array))))
  (local.set $f (struct.get $float 0 (ref.cast (ref $float) (local.get $v))))
...
(func (export "caml_array_of_uniform_array") (export "caml_make_array")
  ...
  (local $init (ref $block)) (local $res (ref $float_array)) (local $size i32)
  (local $i i32)
...
(func (export "caml_floatarray_concat") (param $vl (ref eq)) (result (ref eq))
  (local $i i32) (local $len i32) (local $l (ref eq)) (local $v (ref eq))
  (local $b (ref $block)) (local $fa (ref $float_array))
  (local $fa' (ref $float_array))
  (local.set $l (local.get $vl))
  (local.set $len (i32.const 0))
  (loop $compute_length
      ...
      (block $exit (result (ref eq))
        ...
        (drop
          (block $not_float (result (ref eq))
            ...
            (local.set $len (array.len (local.get $fa')))
            (array.copy $float_array $float_array (local.get $fa)
              (local.get $i) (local.get $fa') (i32.const 0) (local.get $len))
            (local.set $i (i32.add (local.get $i) (local.get $len)))
            (br $fill)))
        (br $fill))))
  (local.get $fa)
...
(func (export "caml_uniform_array_concat")
  (param $vl (ref eq)) (result (ref eq))
  (local $i i32) (local $len i32) (local $l (ref eq)) (local $v (ref eq))
  (local $b (ref $block)) (local $a (ref $block)) (local $a' (ref $block))
  (local.set $l (local.get $vl))
  (local.set $len (i32.const 0))
  (loop $compute_length
    (drop
