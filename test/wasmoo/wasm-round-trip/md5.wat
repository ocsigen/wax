...
(import "io" "caml_getblock"
  (func $caml_getblock (param (ref eq) (ref $bytes) i32 i32) (result i32))
)
...
(type $context
  (struct
    ...
    (field $intermediate (ref $bytes)))
) ;; intermediate buffer
(func $caml_md5_string (export "caml_md5_string") (export "caml_md5_bytes")
  (param $vs (ref eq)) (param $vofs (ref eq)) (param $vlen (ref eq))
  (result (ref eq))
...
(func $caml_md5_chan (export "caml_md5_chan")
  ...
  (local $len i32) (local $buf (ref $bytes)) (local $ctx (ref $context))
  (local $read i32)
...
(func $MD5Update
  ...
  (local.set $len (struct.get $context $len (local.get $ctx)))
  (local.set $in_buf
    (i32.and (i32.wrap_i64 (local.get $len)) (i32.const 0x3f)))
  (struct.set $context $len (local.get $ctx)
      ...
      (if (i32.lt_u (local.get $input_len) (local.get $missing))
        (then
          (array.copy $bytes $bytes
            (struct.get $context $intermediate (local.get $ctx))
            (local.get $in_buf) (local.get $input) (local.get $input_pos)
            (local.get $input_len))
          (return)))
      (array.copy $bytes $bytes
        (struct.get $context $intermediate (local.get $ctx))
        (local.get $in_buf) (local.get $input) (local.get $input_pos)
        (local.get $missing))
      (call $MD5Transform (struct.get $context $w (local.get $ctx))
        (struct.get $context $buffer (local.get $ctx))
        (struct.get $context $intermediate (local.get $ctx)) (i32.const 0))
      (local.set $input_pos
  ...
  (loop $loop
    (if (i32.ge_u (local.get $input_len) (i32.const 64))
      (then
        (call $MD5Transform (struct.get $context $w (local.get $ctx))
          (struct.get $context $buffer (local.get $ctx)) (local.get $input)
          (local.get $input_pos))
...
(func $MD5Final (param $ctx (ref $context)) (result (ref $bytes))
  (local $len i64) (local $in_buf i32) (local $buffer (ref $bytes))
  (local $i i32) (local $res (ref $bytes)) (local $w (ref $int_array))
  (local.set $len (struct.get $context $len (local.get $ctx)))
  (local.set $in_buf
    (i32.and (i32.wrap_i64 (local.get $len)) (i32.const 0x3f)))
  (local.set $buffer (struct.get $context $intermediate (local.get $ctx)))
  (array.set $bytes (local.get $buffer) (local.get $in_buf) (i32.const 0x80))
    ...
    (then
      ...
      (loop $loop
        (if (i32.lt_u (local.get $i) (i32.const 64))
          (then
            (array.set $bytes (local.get $buffer) (local.get $i)
              (i32.const 0))
            (local.set $i (i32.add (local.get $i) (i32.const 1)))
            (br $loop))))
      (call $MD5Transform (struct.get $context $w (local.get $ctx))
        (struct.get $context $buffer (local.get $ctx)) (local.get $buffer)
        (i32.const 0))
  ...
  (local.set $w (struct.get $context $w (local.get $ctx)))
  (loop $loop
