...
(@if (not $wasi)
  (@then
    ...
    (func $caml_realloc_global (export "caml_realloc_global")
      ...
      (local.set $new_len
        (i32.add (i31.get_u (ref.cast (ref i31) (local.get $len)))
          (i32.const 2)))
      (if
    ...
    (@if (>= $ocaml_version (5 2 0))
      (@then
        (func $caml_reify_bytecode (export "caml_reify_bytecode")
          (param $code (ref eq)) (param $debug (ref eq))
          (param $digest (ref eq)) (result (ref eq))
      ...
      (@else
        (func $caml_reify_bytecode (export "caml_reify_bytecode")
          ...
          (local $buf (ref $bytes)) (local $pos i32)
          (local $chunk (ref $bytes)) (local $chunk_len i32)
          (if (ref.test (ref i31) (global.get $toplevel_compile))
            (then
              (call $caml_failwith
                (@string "Toplevel compile callback not initialized"))
          ...
          (array.new_fixed $block 3 (ref.i31 (i32.const 0))
            (ref.i31 (i32.const 0))
            (call $caml_callback_2 (global.get $toplevel_compile)
              (local.get $buf) (local.get $debug))))))

    (func $caml_static_release_bytecode
      (export "caml_static_release_bytecode")
      (param (ref eq)) (result (ref eq))
      (ref.i31 (i32.const 0)))
    ...
    (@if (< $ocaml_version (5 3 0))
      (@then
        (func $caml_get_section_table (export "caml_get_section_table")
          (param (ref eq)) (result (ref eq))
          (local $sections (ref $block)) (local $symb (ref eq))
          (local $crcs (ref eq)) (local $prim (ref eq)) (local $dlpt (ref eq))
          (local.set $sections (call $get_sections))
          (local.set $symb
            (array.get $block (local.get $sections) (i32.const 1)))
          (local.set $crcs
            (array.get $block (local.get $sections) (i32.const 2)))
          (local.set $prim
          ...
          (array.new_fixed $block 3 (ref.i31 (i32.const 0))
            ...
            (array.new_fixed $block 3 (ref.i31 (i32.const 0))
              (array.new_fixed $block 3 (ref.i31 (i32.const 0))
                (@string "CRCS") (local.get $crcs))
              (array.new_fixed $block 3 (ref.i31 (i32.const 0))
                (array.new_fixed $block 3 (ref.i31 (i32.const 0))
                  (@string "PRIM") (local.get $prim))
                (array.new_fixed $block 3 (ref.i31 (i32.const 0))
                  (array.new_fixed $block 3 (ref.i31 (i32.const 0))
                    (@string "DLPT") (local.get $dlpt))
                  (ref.i31 (i32.const 0)))))))))
    ...
    (func $caml_get_current_environment
      (export "caml_get_current_environment")
      (param (ref eq)) (result (ref eq))
      (call $caml_failwith
