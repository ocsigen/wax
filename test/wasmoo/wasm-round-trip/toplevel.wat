...
) (import "bindings" "get_prim_list" (func $get_prim_list (result anyref)))
(import "bindings" "get_crcs" (func $get_crcs (result anyref)))
...
(func $caml_realloc_global (export "caml_realloc_global")
  (param $len (ref eq)) (result (ref eq))
  (local $new_len i32) (local $old (ref $block)) (local $new (ref $block))
  (local.set $new_len
    (i32.add (i31.get_u (ref.cast (ref i31) (local.get $len))) (i32.const 2)))
  (if
    (i32.gt_u (local.get $new_len) (array.len (global.get $caml_global_data)))
    (then
      (local.set $old (global.get $caml_global_data))
      (local.set $new
        (array.new $block (ref.i31 (i32.const 0)) (local.get $new_len)))
      (array.copy $block $block (local.get $new) (i32.const 0)
        (local.get $old) (i32.const 0) (array.len (local.get $old)))
...
(func $wasm_toplevel_init_compile (export "wasm_toplevel_init_compile")
...
(@then
(func $caml_reify_bytecode (export "caml_reify_bytecode")
  ...
  (if (ref.test (ref i31) (global.get $toplevel_compile))
    (then
      ...
      (unreachable)))
  (array.new_fixed $block 3 (ref.i31 (i32.const 0)) (ref.i31 (i32.const 0))
    (call $caml_callback_2 (global.get $toplevel_compile)
      (call $caml_string_of_uint8_array
        (call $caml_ba_to_typed_array (local.get $code))) (local.get $debug)))
) )
(@else
(func $caml_reify_bytecode (export "caml_reify_bytecode")
  (param $code (ref eq)) (param $debug (ref eq)) (param $digest (ref eq))
  (result (ref eq))
  (local $arr (ref $block)) (local $i i32) (local $len i32)
  (local $buf (ref $bytes)) (local $pos i32) (local $chunk (ref $bytes))
  (local $chunk_len i32)
  (if (ref.test (ref i31) (global.get $toplevel_compile))
    (then
      (call $caml_failwith
  ...
  (block $done1
    (loop $loop1
      (br_if $done1 (i32.ge_u (local.get $i) (array.len (local.get $arr))))
      (local.set $len
  ...
  (block $done2
    (loop $loop2
      ...
      (array.copy $bytes $bytes (local.get $buf) (local.get $pos)
        (local.get $chunk) (i32.const 0) (local.get $chunk_len))
      (local.set $pos (i32.add (local.get $pos) (local.get $chunk_len)))
      (local.set $i (i32.add (local.get $i) (i32.const 1)))
      (br $loop2)))
  (array.new_fixed $block 3 (ref.i31 (i32.const 0)) (ref.i31 (i32.const 0))
    (call $caml_callback_2 (global.get $toplevel_compile) (local.get $buf)
      (local.get $debug)))
) ) )

(func $caml_static_release_bytecode (export "caml_static_release_bytecode")
  (param (ref eq)) (result (ref eq))
  (ref.i31 (i32.const 0))
)

(global $nul (ref $bytes) (@string "\00" ))

;; Convert an OCaml string list to a \0-separated string.
;; E.g. ["a"; "b"; "c"] -> "a\x00b\x00c\x00"
(func $string_list_to_sep (param $l (ref eq)) (result (ref eq))
  (local $acc (ref eq)) (local $cell (ref $block))
...
(func $get_sections (result (ref $block))
  ...
  (if (ref.test (ref i31) (local.get $s))
    (then
      (call $caml_failwith (@string "Program not compiled with --toplevel" ))
      (unreachable)))
...
(@then
(func $caml_get_section_table (export "caml_get_section_table")
  (param (ref eq)) (result (ref eq))
  (local $sections (ref $block)) (local $symb (ref eq)) (local $crcs (ref eq))
  (local $prim (ref eq)) (local $dlpt (ref eq))
  ...
  (array.new_fixed $block 3 (ref.i31 (i32.const 0))
    (array.new_fixed $block 3 (ref.i31 (i32.const 0)) (@string "SYMB" )
      (local.get $symb))
    (array.new_fixed $block 3 (ref.i31 (i32.const 0))
      (array.new_fixed $block 3 (ref.i31 (i32.const 0)) (@string "CRCS" )
        (local.get $crcs))
      (array.new_fixed $block 3 (ref.i31 (i32.const 0))
        (array.new_fixed $block 3 (ref.i31 (i32.const 0)) (@string "PRIM" )
          (local.get $prim))
        (array.new_fixed $block 3 (ref.i31 (i32.const 0))
          (array.new_fixed $block 3 (ref.i31 (i32.const 0)) (@string "DLPT" )
            (local.get $dlpt)) (ref.i31 (i32.const 0))))))
) ) )
...
(func $caml_dynlink_get_bytecode_sections
  (export "caml_dynlink_get_bytecode_sections")
  (param (ref eq)) (result (ref eq))
  (call $get_sections)
)

(func $wasm_get_bytecode_sections (export "wasm_get_bytecode_sections")
  (param (ref eq)) (result (ref eq))
  (call $get_sections)
)

(func $wasm_get_runtime_aliases (export "wasm_get_runtime_aliases")
  (param (ref eq)) (result (ref eq))
...
(func $caml_sys_modify_argv (export "caml_sys_modify_argv")
  (param (ref eq)) (result (ref eq))
  ;; no-op: Sys.argv modification is not meaningful in Wasm
  (ref.i31 (i32.const 0))
)

(func $caml_get_current_environment (export "caml_get_current_environment")
  (param (ref eq)) (result (ref eq))
  (call $caml_failwith
...
(func $caml_invoke_traced_function (export "caml_invoke_traced_function")
