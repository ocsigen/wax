...
(@then (import "fail" "caml_failwith" (func $caml_failwith (param (ref eq))))
...
(func $decompress_oneshot
  ...
  (call $bytes_to_mem (local.get $input) (local.get $pos) (local.get $in_buf)
    (local.get $len))
  (if
    (i32.ne
  ...
  (call $mem_to_bytes (local.get $out_buf) (local.get $output) (i32.const 0)
    (local.get $out_len))
  (local.get $output)
)
