...
(@if (>= $ocaml_version (5 1 0))
  (@then
    ...
    (import "zstd" "zstd_stream_set_input"
      (func $stream_set_input (param i32)))
    ...
    (global $decompress_failed (ref $bytes)
      (@string "input_value: zstd decompression failed"))
    ...
    (func $decompress_oneshot
      ...
      (call $bytes_to_mem (local.get $input) (local.get $pos)
        (local.get $in_buf) (local.get $len))
      (if
        (i32.ne
          (call $zstd_decompress (local.get $out_buf) (local.get $out_len)
            (local.get $in_buf) (local.get $len)) (local.get $out_len))
        (then (call $caml_failwith (global.get $decompress_failed))))
      (local.set $output
        (array.new $bytes (i32.const 0) (local.get $out_len)))
      (call $mem_to_bytes (local.get $out_buf) (local.get $output)
        (i32.const 0) (local.get $out_len))
      (local.get $output))
    ...
    (func $decompress_stream
      (param $input (ref $bytes)) (param $pos i32) (param $len i32)
      (param $out_len i32) (result (ref $bytes))
      (local $output (ref $bytes)) (local $src_ofs i32) (local $src_end i32)
      (local $need_input i32) (local $out_ofs i32) (local $avail i32)
      (local $ret i32) (local $produced i32)
        ...
        (loop $loop
          ...
          (if (i32.or (local.get $need_input) (call $in_done))
            (then
              ...
              (call $bytes_to_mem (local.get $input) (local.get $src_ofs)
          ...
          (if
            (i32.gt_u (i32.add (local.get $out_ofs) (local.get $produced))
              (local.get $out_len))
            (then (call $caml_failwith (global.get $decompress_failed))))
          (call $mem_to_bytes (call $out_buf) (local.get $output)
            (local.get $out_ofs) (local.get $produced))
          (local.set $out_ofs
            (i32.add (local.get $out_ofs) (local.get $produced)))
          (br_if $done (i32.eqz (local.get $ret))) ;; frame complete
          (if (call $in_done) (then (local.set $need_input (i32.const 1))))
          (br $loop)))
    ...
    (func $decompress
      ...
      (call $bytes_to_mem (local.get $input) (local.get $pos) (call $in_buf)
        (local.get $hdr))
      (if (result (ref $bytes))
        (i32.le_u (local.get $out_len)
          (call $window_size (call $in_buf) (local.get $hdr)))
        (then
          (call $decompress_oneshot (local.get $input) (local.get $pos)
            (local.get $len) (local.get $out_len)))
        (else
          (call $decompress_stream (local.get $input) (local.get $pos)
            (local.get $len) (local.get $out_len)))))
