...
(func $parse_sign_and_base (export "parse_sign_and_base")
  ...
  (local $i i32) (local $len i32) (local $signedness i32) (local $sign i32)
    ...
    (then
      (if
        ...
        (then
          ...
          (if
            (i32.or (i32.eq (local.get $c) (@char "X" ))
            ...
            (else
              (if
                (i32.or (i32.eq (local.get $c) (@char "O" ))
                ...
                (else
                  (if
                    (i32.or (i32.eq (local.get $c) (@char "B" ))
                    ...
                    (else
                      (if
                        (i32.or (i32.eq (local.get $c) (@char "U" ))
...
(func $parse_int (export "parse_int")
  (param $v (ref eq)) (param $nbits i32) (param $errmsg (ref eq)) (result i32)
  (local $s (ref $bytes)) (local $len i32) (local $base i32) (local $sign i32)
  (local $signedness i32) (local $i i32) (local $threshold i32) (local $d i32)
  (local $res i32) (local $c i32)
...
(global $INT_ERRMSG (ref $bytes) (@string "int_of_string" ))
...
(global $INT8_ERRMSG (ref $bytes) (@string "Int8.of_string" ))

(func $caml_int8_of_string (export "caml_int8_of_string")
  (param $v (ref eq)) (result (ref eq))
  (ref.i31
    (i32.extend8_s
      (call $parse_int (local.get $v) (i32.const 8)
        (global.get $INT8_ERRMSG))))
)

(global $INT16_ERRMSG (ref $bytes) (@string "Int16.of_string" ))

(func $caml_int16_of_string (export "caml_int16_of_string")
  (param $v (ref eq)) (result (ref eq))
  (ref.i31
    (i32.extend16_s
      (call $parse_int (local.get $v) (i32.const 16)
        (global.get $INT16_ERRMSG))))
)
...
(func $format_int_default (param $d i32) (result (ref eq))
  (local $negative i32) (local $i i32) (local $n i32) (local $s (ref $bytes))
...
(global $format_error (ref $bytes) (@string "format_int: bad format" ))
...
(func $parse_int_format (export "parse_int_format")
  ...
  (local $uppercase i32) (local $c i32)
  (local.set $len (array.len (local.get $s)))
  (local.set $i (i32.const 1))
  (block $return
    (block $bad_format
      ...
      (if
        (i32.or
          (i32.or (i32.eq (local.get $c) (@char "L" ))
      ...
      (if
        (i32.or (i32.eq (local.get $c) (@char "d" ))
          (i32.eq (local.get $c) (@char "i" )))
        (then
          (local.set $base (i32.const 10))
          (local.set $signed (i32.const 1)))
        (else
          (if (i32.eq (local.get $c) (@char "u" ))
            (then (local.set $base (i32.const 10)))
            (else
              (if (i32.eq (local.get $c) (@char "x" ))
...
(func $format_int (export "format_int")
  ...
  (local $s (ref $bytes)) (local $uppercase i32) (local $base i32)
  (local $signed i32) (local $alternate i32) (local $sign_style i32)
  (local $negative i32) (local $n i32) (local $i i32)
  ...
  ;; base 8/16.
  (if
    (i32.and (local.get $alternate) (i32.ne (local.get $base) (i32.const 10)))
    (then
      (if (local.get $i)
        (then
