      ...
      (if (i32.gt_s (local.get $prec) (i32.const 0))
        (then
          ...
          (loop $write
            ...
            (local.set $i (i32.add (local.get $i) (i32.const 1)))
            (br_if $write (i32.gt_s (local.get $prec) (i32.const 0)))) ;; 'write
        ))
...
(func $parse_format (param $s (ref $string)) (result i32 i32 i32 i32)
  ...
  (block $return
    (block $bad_format
      ...
      (br_if $return (i32.le_u (local.get $conversion) (i32.const 2)))) ;; 'bad_format
  ...
  (if (local.get $uppercase)
    (then
      ...
      (loop $uppercase
        ...
        (local.set $i (i32.add (local.get $i) (i32.const 1)))
        (br_if $uppercase (i32.lt_u (local.get $i) (local.get $len)))) ;; 'uppercase
    ))
  (local.get $s)
)
                  ...
                  (if (i32.ne (local.get $i) (local.get $len))
                    (then
                      (local.set $c
                        (array.get_u $string (local.get $s) (local.get $i)))
                      (local.set $i (i32.add (local.get $i) (i32.const 1)))
                      (br $parse_exponent)))) ;; 'parse_exponent
                (if (local.get $negative)
                  (then
                    (br_if $overflow
...
(func $caml_float_of_string (export "caml_float_of_string")
  ...
  (block $error
    ...
    (if (i32.eq (i32.add (local.get $i) (i32.const 3)) (local.get $len))
      (then
        (local.set $c (array.get_u $string (local.get $s) (local.get $i)))
        (if (i32.eq (i32.and (local.get $c) (i32.const 0xdf)) (i32.const 78)) ;; 'N'
          (then
            (local.set $i (i32.add (local.get $i) (i32.const 1)))
            (local.set $c (array.get_u $string (local.get $s) (local.get $i)))
            (if
        ...
        (if (i32.eq (i32.and (local.get $c) (i32.const 0xdf)) (i32.const 73)) ;; 'I'
          (then
            (local.set $i (i32.add (local.get $i) (i32.const 1)))
            (local.set $c (array.get_u $string (local.get $s) (local.get $i)))
            (if
    ...
    (return (struct.new $float (local.get $f)))) ;; 'error
  (call $caml_failwith
    (array.new_data $string $float_of_string (i32.const 0) (i32.const 15)))
...
(func $caml_modf_float (export "caml_modf_float")
  ...
  (if (f64.ge (local.get $a) (f64.const 0))
    (then
      (if (f64.lt (local.get $a) (f64.const inf)) ;; normal
        (then
        ...
        (else
          ...
          (local.set $f (f64.copysign (f64.const 0) (local.get $x_2))))) ;; zero or nan
    )
  ...
  (block $l_2
    ...
    (block $cont
      (block $then
        ...
        (br_if $cont (i64.ne (local.get $x_7) (i64.const 2047)))) ;; 'then
      (return
        (struct.new $float
          (f64.add (f64.mul (local.get $x) (local.get $y)) (local.get $z))))) ;; 'cont
    (block $cont
        ...
        (block $l_5
          ...
          (local.set $y
            (select (f64.mul (local.get $y) (f64.const 0x1p-53))
              (local.get $y) (i64.gt_u (local.get $x_3) (i64.const 53))))) ;; 'l_5
        (local.set $z (f64.mul (local.get $z) (f64.const 0x1p-53)))) ;; 'l_4
      (local.set $x_9 (i32.const 0))
