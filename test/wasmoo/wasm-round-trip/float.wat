...
(global $nan_chars (ref $chars) (@string $chars "nan"))

(func $Double_val (export "Double_val") (param $v (ref eq)) (result f64)
  (struct.get $float $f (ref.cast (ref $float) (local.get $v)))
)

(func $caml_hexstring_of_float (export "caml_hexstring_of_float")
  (param $v (ref eq)) (param $vprec (ref eq)) (param $vstyle (ref eq))
  (result (ref eq))
  (local $prec i32) (local $style i32) (local $b i64) (local $sign i32)
  (local $exp i32) (local $m i64) (local $i i32) (local $s (ref $bytes))
  (local $len i32) (local $txt (ref $chars)) (local $frac i64)
  (local $unit i64) (local $half i64) (local $mask i64) (local $j i32)
  (local $d i32)
...
(global $format_error (ref $bytes) (@string "format_float: bad format"))

(func $parse_format (param $s (ref $bytes)) (result i32 i32 i32 i32)
  ...
  (block $return
    (block $bad_format
      ...
      (if
        (i32.eq (array.get_u $bytes (local.get $s) (local.get $i))
          (@char "."))
        (then
          (loop $precision
            (local.set $i (i32.add (local.get $i) (i32.const 1)))
...
(global $inf_chars (ref $chars) (@string $chars "inf"))

(@if $wasi
  (@then
    (func $caml_format_float (export "caml_format_float")
      (param $vfmt (ref eq)) (param $arg (ref eq)) (result (ref eq))
      (local $fmt (ref $bytes)) (local $d f64) (local $buffer i32)
      (local $fmt_len i32) (local $out_buffer i32) (local $avail i32)
      (local $len i32) (local $res (ref $bytes))
      (local.set $fmt (ref.cast (ref $bytes) (local.get $vfmt)))
      (local.set $d
        (struct.get $float $f (ref.cast (ref $float) (local.get $arg))))
      ...
      (i32.store8 $m (i32.add (local.get $buffer) (local.get $fmt_len))
        (i32.const 0))
      ...
      (local.set $len
        (call $format_float (local.get $out_buffer) (local.get $avail)
      ...
      (if (i32.ge_u (local.get $len) (local.get $avail))
        (then
          ...
          (drop
            (call $format_float (local.get $out_buffer)
              (i32.add (local.get $len) (i32.const 1)) (local.get $buffer)
              (local.get $d)))))
      (local.set $res
        (call $blit_memory_to_string (local.get $out_buffer)
          (local.get $len)))
      (if (i32.ge_u (local.get $len) (local.get $avail))
        (then (call $free (local.get $out_buffer))))
      (local.get $res)))
  (@else
    (func $caml_format_float (export "caml_format_float")
      (param $vfmt (ref eq)) (param $arg (ref eq)) (result (ref eq))
      ...
      (local $negative i32) (local $i i32) (local $len i32)
      (local $s (ref $bytes)) (local $exp i32) (local $m i64)
      (local $txt (ref $chars)) (local $num anyref) (local $c i32)
      (local.set $f
        (struct.get $float $f (ref.cast (ref $float) (local.get $arg))))
      (local.set $b (i64.reinterpret_f64 (local.get $f)))
      (call $parse_format (ref.cast (ref $bytes) (local.get $vfmt)))
      (local.set $uppercase)
      (local.set $conversion)
      (local.set $precision)
        ...
        (then
          ...
          (if (i32.eq (local.get $c) (@char "."))
            (then
              ...
              (br $parse)))
          (if
            (i32.or (i32.eq (local.get $c) (@char "p"))
            ...
            (then
              ...
              (block $overflow
                (loop $parse_exponent
                  ...
                  (br_if $overflow
                    (i32.ge_u (local.get $exp) (i32.const 0x10000000)))
          ...
          (if
            (i32.and (i32.ge_u (local.get $c) (@char "0"))
              (i32.le_u (local.get $c) (@char "9")))
            (then (local.set $d (i32.sub (local.get $c) (@char "0"))))
            (else
              (if
                (i32.and (i32.ge_u (local.get $c) (@char "a"))
...
(func $caml_parse_float (export "caml_parse_float")
  (param $err_msg (ref eq)) (param $v (ref eq)) (result (ref eq))
  (local $s (ref $bytes)) (local $len i32) (local $i i32) (local $j i32)
  (local $c i32) (local $s' (ref $bytes)) (local $start i32)
  (local $negative i32) (local $f f64) (local $buffer i32) (local $buf i32)
  ...
  (block $error
        ...
        (if
          ...
          (then
            (if
              ...
              (then
                (local.set $f
                  (call $caml_float_of_hex (local.get $err_msg) (local.get $s)
    ...
    (if (i32.eq (i32.add (local.get $i) (i32.const 3)) (local.get $len))
      (then
        (local.set $c (array.get_u $bytes (local.get $s) (local.get $i)))
        (if (i32.eq (i32.and (local.get $c) (i32.const 0xdf)) (@char "N"))
          (then
            (local.set $i (i32.add (local.get $i) (i32.const 1)))
            (local.set $c (array.get_u $bytes (local.get $s) (local.get $i)))
            (if (i32.eq (i32.and (local.get $c) (i32.const 0xdf)) (@char "A"))
              (then
                (local.set $i (i32.add (local.get $i) (i32.const 1)))
                (local.set $c
                  (array.get_u $bytes (local.get $s) (local.get $i)))
          ...
          (else
            (if (i32.eq (i32.and (local.get $c) (i32.const 0xdf)) (@char "I"))
              (then
                (local.set $i (i32.add (local.get $i) (i32.const 1)))
                (local.set $c
                  (array.get_u $bytes (local.get $s) (local.get $i)))
...
(global $float_of_string (ref $bytes) (@string "float_of_string"))

(func $caml_float_of_string (export "caml_float_of_string")
  (param $s (ref eq)) (result (ref eq))
  (return_call $caml_parse_float (global.get $float_of_string) (local.get $s))
)
...
(func $caml_modf_float (export "caml_modf_float")
  ...
  (local.set $x (struct.get $float $f (ref.cast (ref $float) (local.get $v))))
  (local.set $a (f64.abs (local.get $x)))
...
(func $frexp (param $x f64) (result f64 i32)
  (local $y i64) (local $e i32) (local $r_1 i32) (local $r_0 f64)
    ...
    (then
      (if (f64.ne (local.get $x) (f64.const 0))
        (then
          ...
          (local.set $r_1)
          (local.set $r_0)
          (return (local.get $r_0) (i32.sub (local.get $r_1) (i32.const 64))))
...
(func $caml_frexp_float (export "caml_frexp_float")
  (param $v (ref eq)) (result (ref eq))
  (local $r_1 i32) (local $r_0 f64)
  (call $frexp (struct.get $float $f (ref.cast (ref $float) (local.get $v))))
  (local.set $r_1)
  (local.set $r_0)
  (array.new_fixed $block 3 (ref.i31 (i32.const 0))
    (struct.new $float (local.get $r_0)) (ref.i31 (local.get $r_1)))
)
...
(func $caml_fma_float (export "caml_fma_float")
  ...
  (result (ref eq))
  (local $x f64) (local $y f64) (local $z f64) (local $v3 i64) (local $v4 i64)
  (local $v5 i64) (local $v6 i64) (local $v7 i64) (local $v8 i64)
  (local $v9 i32) (local $v10 i32) (local $v11 f64) (local $v12 f64)
  (local $v13 f64) (local $v14 f64) (local $v15 f64)
  (local.set $x
  ...
  (local.set $v7
    (i64.add
      (local.tee $v4
        (i64.and
          (i64.shr_u (local.tee $v3 (i64.reinterpret_f64 (local.get $y)))
            (i64.const 52)) (i64.const 2047)))
      (local.tee $v6
        (i64.and
          (i64.shr_u (local.tee $v5 (i64.reinterpret_f64 (local.get $x)))
            (i64.const 52)) (i64.const 2047)))))
  (local.set $v8 (i64.reinterpret_f64 (local.get $z)))
  (block $label_1
    (block $label_2
      (br_if $label_2 (i64.gt_u (local.get $v4) (i64.const 1993)))
      (br_if $label_2 (i64.gt_u (local.get $v6) (i64.const 1993)))
      (br_if $label_2 (i64.gt_u (local.get $v7) (i64.const 3016)))
      (br_if $label_2
        (i64.gt_u (i64.and (local.get $v8) (i64.const 0x7fe0000000000000))
          (i64.const 0x7c90000000000000)))
      (local.set $v9 (i32.const 0))
      (br_if $label_2 (i64.le_u (local.get $v7) (i64.const 1076)))
      (local.set $v10 (i32.const 0))
      (br $label_1))
    (local.set $v8
      (i64.and (i64.shr_u (local.get $v8) (i64.const 52)) (i64.const 2047)))
    (block $cont
      (br_if $cont (i64.eq (local.get $v4) (i64.const 2047)))
      (br_if $cont (i64.eq (local.get $v6) (i64.const 2047)))
      (br_if $cont (i64.ne (local.get $v8) (i64.const 2047)))
      (return
        (struct.new $float
    ...
    (block $cont
      (block $then
        (br_if $then (i64.eq (local.get $v6) (i64.const 2047)))
        (br_if $then (i64.eq (local.get $v4) (i64.const 2047)))
    ...
    (block $cont
      (br_if $cont (i64.lt_u (local.get $v7) (i64.const 3071)))
      (return (struct.new $float (f64.mul (local.get $x) (local.get $y)))))
    (block $cont
      (br_if $cont (i64.gt_u (local.get $v7) (i64.const 967)))
      (local.set $y
        (select (f64.const 0x1p-1074) (f64.const -0x1p-1074)
          (i64.gt_s (i64.xor (local.get $v3) (local.get $v5))
            (i64.const -1))))
      (block $cont2
        (br_if $cont2 (i64.lt_u (local.get $v8) (i64.const 3)))
    ...
    (block $label_10
      (block $label_11
        (block $label_12
          (br_if $label_12 (i64.lt_u (local.get $v7) (i64.const 3017)))
          (local.set $z
            (select (f64.mul (local.get $z) (f64.const 0x1p-53))
              (local.get $z) (i64.gt_u (local.get $v8) (i64.const 53))))
          (local.set $x
            (select (f64.mul (local.get $x) (f64.const 0x1p-53))
              (local.get $x)
              (local.tee $v9 (i64.gt_u (local.get $v6) (local.get $v4)))))
          (local.set $y
            (select (local.get $y)
              (f64.mul (local.get $y) (f64.const 0x1p-53)) (local.get $v9)))
          (br $label_11))
        (br_if $label_10 (i64.lt_u (local.get $v8) (i64.const 1994)))
        (block $label_13
          (block $label_14
            (br_if $label_14 (i64.gt_u (local.get $v7) (i64.const 1129)))
            (block $label_15
              (br_if $label_15 (i64.le_u (local.get $v6) (local.get $v4)))
              (local.set $x (f64.mul (local.get $x) (f64.const 0x1p108)))
              (br $label_13))
            (local.set $y (f64.mul (local.get $y) (f64.const 0x1p108)))
            (br $label_13))
          (block $label_16
            (br_if $label_16 (i64.le_u (local.get $v6) (local.get $v4)))
            (local.set $x
              (select (f64.mul (local.get $x) (f64.const 0x1p-53))
                (local.get $x) (i64.gt_u (local.get $v6) (i64.const 53))))
            (br $label_13))
          (local.set $y
            (select (f64.mul (local.get $y) (f64.const 0x1p-53))
              (local.get $y) (i64.gt_u (local.get $v4) (i64.const 53)))))
        (local.set $z (f64.mul (local.get $z) (f64.const 0x1p-53))))
      (local.set $v10 (i32.const 0))
      (local.set $v9 (i32.const 1))
      (br $label_1))
    (block $label_17
      (block $label_18
        (br_if $label_18 (i64.lt_u (local.get $v6) (i64.const 1994)))
        (local.set $y (f64.mul (local.get $y) (f64.const 0x1p53)))
        (local.set $x (f64.mul (local.get $x) (f64.const 0x1p-53)))
        (br $label_17))
      (block $label_19
        (br_if $label_19 (i64.lt_u (local.get $v4) (i64.const 1994)))
      ...
      (local.set $z
        (select (f64.mul (local.get $z) (f64.const 0x1p108)) (local.get $z)
          (local.tee $v10 (i64.lt_u (local.get $v8) (i64.const 219)))))
      (local.set $x
          ...
          (f64.sub (local.get $x)
            (local.tee $v11
              (f64.sub
                ...
                (f64.sub (local.get $v11) (local.get $x))))))
        (local.tee $v14
          (f64.sub (local.get $y)
            (local.tee $v13
              (f64.sub
                ...
                (f64.sub (local.get $v13) (local.get $y)))))))
      (f64.sub
        (f64.sub
          (f64.sub (local.tee $v15 (f64.mul (local.get $y) (local.get $x)))
            (f64.mul (local.get $v11) (local.get $v13)))
          (f64.mul (local.get $v12) (local.get $v13)))
        (f64.mul (local.get $v11) (local.get $v14)))))
  (block $label_21
    (block $label_22
      (br_if $label_22
        (f64.ne (local.tee $y (f64.add (local.get $z) (local.get $v15)))
          (f64.const 0)))
                      ...
                      (f64.sub
                        (local.tee $z
                          (f64.add
                            (local.tee $v11
                              (f64.add
                                (f64.sub (local.get $v15)
                                  (local.tee $v11
                                    (f64.sub (local.get $y) (local.get $z))))
                                (f64.sub (local.get $z)
                                  (f64.sub (local.get $y) (local.get $v11)))))
                            (local.get $x))) (local.get $v11))))
                  (f64.sub (local.get $v11)
                    (f64.sub (local.get $z) (local.get $v13)))))
              (f64.sub
                (local.tee $y
                  (f64.add
                    (f64.sub (local.get $z)
                      (local.tee $v13
                        (f64.sub
                          ...
                          (local.get $y))))
                    (f64.sub (local.get $y)
                      (f64.sub (local.get $x) (local.get $v13)))))
                (local.tee $y (f64.add (local.get $v11) (local.get $y))))))
          (f64.const 0)))
      (br_if $cont
        (i32.and
          (i32.wrap_i64 (local.tee $v4 (i64.reinterpret_f64 (local.get $y))))
          (i32.const 1)))
      (local.set $y
        (f64.reinterpret_i64
          (i64.add
            (select (i64.const 1) (i64.const -1)
              (i32.xor (f64.lt (local.get $y) (f64.const 0))
                (f64.gt (local.get $z) (f64.const 0)))) (local.get $v4)))))
    (local.set $y (f64.add (local.get $x) (local.get $y)))
    (block $cont
      (br_if $cont (i32.eqz (local.get $v9)))
      (return
...
(func $caml_round_float (export "caml_round_float")
