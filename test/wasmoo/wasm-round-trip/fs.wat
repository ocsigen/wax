...
(@else (import "bindings" "on_windows" (global $on_windows i32))
...
(import "bindings" "is_file" (func $is_file (param anyref) (result (ref eq))))
...
(import "bindings" "register_file"
  (func $register_file (param anyref anyref))
)
...
(func $normalize_prefix (param $prefix (ref $bytes)) (result (ref $bytes))
  (local $len i32) (local $i i32) (local $c i32) (local $res (ref $bytes))
  (local.set $len (array.len (local.get $prefix)))
  (loop $loop
    (if (i32.lt_u (local.get $i) (local.get $len))
      (then
        (local.set $c (array.get_u $bytes (local.get $prefix) (local.get $i)))
              ...
              (else
                ...
                (if (i32.eq (local.get $c) (i32.const 47)) ;; '/'
                  (then
                    ...
                    (br $loop))))))))))
  (if (i32.eq (local.get $i) (local.get $len)) (then (return (@string "" ))))
  ...
  (if (i32.gt_u (local.get $i) (i32.const 1))
    (then
      ...
      (array.copy $bytes $bytes (local.get $res) (i32.const 0)
...
(func $get_preopens (result (ref null $preopen))
  (local $buffer i32) (local $fd i32) (local $res i32) (local $len i32)
          ...
          (br_if $skip
            (i32.eqz
              (i32.and (i32.eqz (local.get $res))
                (i32.eqz (i32.load8_u $m (local.get $buffer))))))
          (local.set $len (i32.load $m offset=4 (local.get $buffer)))
          (local.set $res
            (call $fd_prestat_dir_name (local.get $fd) (local.get $buffer)
              (local.get $len)))
          (br_if $skip (local.get $res))
          (global.set $preopens
            (struct.new $preopen
              (call $normalize_prefix
                (call $blit_memory_to_string (local.get $buffer)
                  (local.get $len))) (local.get $fd) (global.get $preopens))))
...
(global $root_dir (ref $bytes) (@string "/" ))

(func $make_absolute (param $path (ref $bytes)) (result (ref $bytes))
  (local $i i32) (local $need_slash i32) (local $abs_path (ref $bytes))
      ...
      (then
        (i32.ne (i32.const 47) ;; '/'
          (array.get_u $bytes (global.get $current_dir)
            (i32.sub (array.len (global.get $current_dir)) (i32.const 1)))))
      (else (i32.const 1))))
  (local.set $abs_path
    (array.new $bytes (i32.const 0)
  ...
  (array.copy $bytes $bytes (local.get $abs_path) (i32.const 0)
    (global.get $current_dir) (i32.const 0)
    (array.len (global.get $current_dir)))
  (array.set $bytes (local.get $abs_path)
    (array.len (global.get $current_dir)) (i32.const 47)) ;; '/'
  (array.copy $bytes $bytes (local.get $abs_path)
    (i32.add (array.len (global.get $current_dir)) (local.get $need_slash))
...
(func $wasi_chdir (export "wasi_chdir") (param $name (ref eq))
  (local $abs_path (ref $bytes)) (local $i i32) (local $path (ref $bytes))
  (local.set $abs_path
  ...
  (if (i32.lt_u (local.get $i) (array.len (local.get $abs_path)))
    (then
      ...
      (array.copy $bytes $bytes (local.get $path) (i32.const 0)
        (local.get $abs_path) (i32.const 0) (local.get $i))
...
(func $prefix_match
  ...
  (if (i32.gt_u (local.get $len) (i32.const 0))
    (then
      (if
        (i32.eq
          (array.get_u $bytes (local.get $prefix)
            (i32.sub (local.get $len) (i32.const 1))) (i32.const 47)) ;; '/'
  ...
  (if (i32.gt_u (array.len (local.get $path)) (local.get $len))
    (then
      (if
        (i32.ne (array.get_u $bytes (local.get $path) (local.get $len))
          (i32.const 47))
        (then (return (i32.const 0))))))
  (loop $loop
    (if (i32.lt_u (local.get $i) (local.get $len))
      (then
        (if
          (i32.ne (array.get_u $bytes (local.get $path) (local.get $i))
...
(func $resolve_abs_path (param $path (ref $bytes)) (result i32 (ref $bytes))
  (local $preopens (ref null $preopen)) (local $i i32) (local $fd i32)
  (local $current (ref $preopen)) (local $prefix (ref $bytes))
  (local $len i32) (local $rel_path (ref $bytes))
  (local.set $preopens (call $get_preopens))
  (local.set $i (i32.const -1))
  (block $done
    (loop $loop
      (local.set $current (br_on_null $done (local.get $preopens)))
      (local.set $prefix (struct.get $preopen $prefix (local.get $current)))
      (if
        (i32.and (i32.gt_s (array.len (local.get $prefix)) (local.get $i))
      ...
      (local.set $preopens (struct.get $preopen $next (local.get $current)))
      (br $loop)))
  ...
  (loop $loop
    (if (i32.lt_u (local.get $i) (array.len (local.get $path)))
      (then
        (if
          (i32.eq (array.get_u $bytes (local.get $path) (local.get $i))
            (i32.const 47)) ;; 47
          (then
            (local.set $i (i32.add (local.get $i) (i32.const 1)))
            (br $loop)))
        (local.set $rel_path
          (array.new $bytes (i32.const 0)
            (i32.sub (array.len (local.get $path)) (local.get $i))))
        (array.copy $bytes $bytes (local.get $rel_path) (i32.const 0)
          (local.get $path) (local.get $i)
          (i32.sub (array.len (local.get $path)) (local.get $i)))
        (return (local.get $fd) (local.get $rel_path)))))
  (return (local.get $fd) (@string "." ))
)

(func $wasi_resolve_path (export "wasi_resolve_path")
  (param $vpath (ref eq)) (;fd;) (;address;) (;length;) (result i32 i32 i32)
  (local $res_1 (ref $bytes)) (local $res_0 i32) (local $p i32)
  ...
  (call $resolve_abs_path
    (call $make_absolute (ref.cast (ref $bytes) (local.get $vpath))))
  (local.set $res_1)
  (local.set $res_0)
    ...
    (then
      (local.set $p
        (call $write_string_to_memory (i32.const 0) (i32.const 0)
          (local.get $res_1)))))
  (return (local.get $res_0) (local.get $p) (array.len (local.get $res_1)))
)
...
(func $caml_sys_getcwd (export "caml_sys_getcwd") (export "unix_getcwd")
  (export "caml_unix_getcwd") (param (ref eq)) (result (ref eq))
...
(func $caml_sys_chdir (export "caml_sys_chdir")
  ...
  (if (i32.ne (local.get $kind) (i32.const 3))
    (then
      (call $caml_handle_sys_error (local.get $name)
        (i32.const 54)))) ;; ENOTDIR
...
(func $caml_sys_mkdir (export "caml_sys_mkdir")
  ...
  (return_call $sys_path_op (local.get $name)
...
(func $caml_sys_read_directory (export "caml_sys_read_directory")
  (param $name (ref eq)) (result (ref eq))
  (local $p_len i32) (local $p_addr i32) (local $p_fd i32) (local $buffer i32)
  (local $res i32) (local $fd i32) (local $buf i32) (local $size i32)
  (local $tbl (ref $block)) (local $i i32) (local $pos i32)
  (local $available i32) (local $new_tbl (ref $block)) (local $entry_size i32)
  (local $cookie i64) (local $left i32) (local $entry i32)
  (local $namelen i32) (local $s (ref $bytes)) (local $new_buf i32)
  (call $caml_sys_resolve_path (local.get $name))
  (local.set $p_len)
  (local.set $p_addr)
  (local.set $p_fd)
  ...
  (loop $loop
    (block $refill
      ...
      (if (i32.eq (local.get $namelen) (i32.const 2))
        ...
        (else
          (if (i32.eq (local.get $namelen) (i32.const 1))
            (then
              (br_if $loop
                (i32.eq (array.get_u $bytes (local.get $s) (i32.const 0))
                  (i32.const 46)))))))
    ...
    (if (i32.lt_u (local.get $size) (local.get $entry_size))
      (then
        ...
        (local.set $new_buf (call $checked_malloc (local.get $entry_size)))
    ...
    (block $done
      (br_if $done
        (i32.and (i32.ne (i32.const 0) (local.get $available))
          (i32.lt_u (local.get $available) (local.get $size))))
      (local.set $res
        (call $fd_readddir (local.get $fd) (local.get $buf) (local.get $size)
          (local.get $cookie) (local.get $buffer)))
      (if (local.get $res)
        (then
          (call $free (local.get $buf))
...
(@if $wasi
(@then
(func $caml_sys_rmdir (export "caml_sys_rmdir")
  (param $name (ref eq)) (result (ref eq))
  (return_call $sys_path_op (local.get $name)
...
(func $caml_sys_remove (export "caml_sys_remove")
  (param $name (ref eq)) (result (ref eq))
  (return_call $sys_path_op (local.get $name) (ref.func $path_unlink_file))
) )
(@else
(func $caml_sys_remove (export "caml_sys_remove")
  (param $name (ref eq)) (result (ref eq))
...
(func $caml_sys_file_exists (export "caml_sys_file_exists")
  ...
  (if (i32.lt_s (local.get $p_fd) (i32.const 0))
    (then (call $free (local.get $p_addr)) (return (ref.i31 (i32.const 0)))))
  (local.set $buffer (call $get_buffer))
  (local.set $res
    (call $path_filestat_get (local.get $p_fd) (i32.const 1)
      (local.get $p_addr) (local.get $p_len) (local.get $buffer)))
...
(global $no_such_file (ref $bytes) (@string ": No such file or directory" ))

(func $caml_raise_no_such_file (param $name (ref eq))
  (call $caml_raise_sys_error
    (call $caml_string_concat (local.get $name) (global.get $no_such_file)))
)
...
(func $caml_read_file_content (export "caml_read_file_content")
  ...
  (return_call $caml_string_of_uint8_array (call $wrap (local.get $res)))
) ) )

(@if $wasi
(@then
(func $caml_create_file (export "caml_create_file")
...
(func $caml_sys_file_mode (param $name (ref eq)) (result i32)
  ...
  (call $free (local.get $p_addr))
  (call $caml_handle_sys_error_if (local.get $name) (local.get $res))
  (i32.load8_u $m offset=16 (local.get $buffer))
)
...
(@if $wasi
(@then
(func $caml_sys_is_regular_file (export "caml_sys_is_regular_file")
  (param $name (ref eq)) (result (ref eq))
  (ref.i31
...
(@if $wasi
(@then
(func $caml_sys_temp_dir_name (export "caml_sys_temp_dir_name")
  (param (ref eq)) (result (ref eq))
  (@string "/tmp" )
) )
(@else
(func $caml_sys_temp_dir_name (export "caml_sys_temp_dir_name")
  ...
  (if (global.get $on_windows)
    (then (return_call $caml_string_of_jsstring (call $wrap (call $tmpdir)))))
  (@string "" )
) ) )

(func $caml_mount_autoload (export "caml_mount_autoload")
  (param (ref eq) (ref eq)) (result (ref eq))
  (ref.i31 (i32.const 0))
)

(func $caml_list_mount_point (export "caml_list_mount_point")
  (param (ref eq)) (result (ref eq))
  (ref.i31 (i32.const 0))
)

(func $caml_unmount (export "caml_unmount") (param (ref eq)) (result (ref eq))
  (ref.i31 (i32.const 0))
)
