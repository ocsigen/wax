  ...
  (@then
    ...
    (func $make_absolute (param $path (ref $bytes)) (result (ref $bytes))
      ...
      (array.copy $bytes $bytes (local.get $abs_path)
        (i32.add (array.len (global.get $current_dir))
          (local.get $need_slash)) (local.get $path) (local.get $i)
    ...
    (func $wasi_resolve_path (export "wasi_resolve_path")
      (param $vpath (ref eq)) (;fd;) (;address;) (;length;)
      (result i32 i32 i32)
        ...
        (then
          (local.set $p
            (call $write_string_to_memory (i32.const 0) (i32.const 0)
              (local.get $res_1)))))
      (return (local.get $res_0) (local.get $p)
...
(@if $wasi
  (@then
    (func $caml_sys_read_directory (export "caml_sys_read_directory")
      (param $name (ref eq)) (result (ref eq))
      (local $p_len i32) (local $p_addr i32) (local $p_fd i32)
      (local $buffer i32) (local $res i32) (local $fd i32) (local $buf i32)
      (local $size i32) (local $tbl (ref $block)) (local $i i32)
      (local $pos i32) (local $available i32) (local $new_tbl (ref $block))
      (local $entry_size i32) (local $cookie i64) (local $left i32)
      (local $entry i32) (local $namelen i32) (local $s (ref $bytes))
      (local $new_buf i32)
      (call $caml_sys_resolve_path (local.get $name))
      (local.set $p_len)
      (local.set $p_addr)
      ...
      (local.set $tbl
        (array.new $block (ref.i31 (i32.const 0)) (i32.const 50)))
      (local.set $i (i32.const 1))
      (loop $loop
        (block $refill
        ...
        (block $done
          ...
          (local.set $res
            (call $fd_readddir (local.get $fd) (local.get $buf)
              (local.get $size) (local.get $cookie) (local.get $buffer)))
          (if (local.get $res)
            (then
              (call $free (local.get $buf))
  ...
  (@else
    (func $caml_create_file (export "caml_create_file")
      ...
      (call $register_file
        (call $unwrap (call $caml_jsstring_of_string (local.get $name)))
        (call $unwrap
          (call $caml_uint8_array_of_string (local.get $content))))
      (ref.i31 (i32.const 0))))
)
