...
(type $closure_last_arg
  (sub $closure (struct (field $func (ref $function_1))))
)
...
(type $dummy_closure_1
  (sub final $closure_last_arg
    (struct
      ...
      (field $closure (mut (ref null $closure)))))
)
...
(func $caml_obj_is_stack (export "caml_obj_is_stack")
  (param (ref eq)) (result (ref eq))
  (ref.i31 (i32.const 0))
)

(func $caml_succ_scannable_prefix_len
  (export "caml_succ_scannable_prefix_len") (param (ref eq)) (result (ref eq))
  (ref.i31 (i32.const 0))
)

(global $unique_words_unsupported (ref $bytes)
  (@string "Obj.uniquely_reachable_words is not available in wasm." )
)

(func $caml_obj_uniquely_reachable_words
  (export "caml_obj_uniquely_reachable_words")
  (param (ref eq)) (result (ref eq))
...
(func $caml_alloc_dummy_mixed (export "caml_alloc_dummy_mixed")
...
(func $caml_alloc_dummy_lazy (export "caml_alloc_dummy_lazy")
  (param (ref eq)) (result (ref eq))
  (array.new_fixed $block 2 (ref.i31 (i32.const 0)) (ref.i31 (i32.const 0)))
)

(func $caml_update_dummy_lazy (export "caml_update_dummy_lazy")
  (param $dummy (ref eq)) (param $newval (ref eq)) (result (ref eq))
  (local $tg i32) (local $b (ref $block))
  (local.set $tg
...
(func $caml_obj_dup (export "caml_obj_dup")
  ...
  (local $len i32) (local $s (ref $bytes)) (local $forig (ref $float_array))
  (local $orig (ref $block)) (local $res (ref $block))
  (local $fres (ref $float_array)) (local $s' (ref $bytes))
  ...
  (if (ref.test (ref i31) (local.get $v)) (then (return (local.get $v))))
  (block $default
    (local.set $s
      (block $arm_2 (result (ref $bytes))
        (local.set $forig
          (block $arm_1 (result (ref $float_array))
            (local.set $orig
              (block $arm (result (ref $block))
                (drop
                  (br_on_cast $arm_2 (ref eq) (ref $bytes)
    ...
    (array.copy $bytes $bytes (local.get $s') (i32.const 0) (local.get $s)
...
(func $caml_obj_block (export "caml_obj_block")
  ...
  (local $n i32) (local $res (ref $block))
  ...
  (if
    (i32.eq (i31.get_s (ref.cast (ref i31) (local.get $tg)))
      (global.get $double_array_tag))
    (then (return (array.new $float_array (f64.const 0) (local.get $n)))))
  (local.set $res
    (array.new $block (ref.i31 (i32.const 0))
...
(func $obj_update_tag
  (param $v (ref eq)) (param $o i32) (param $n i32) (result i32)
  (local $b (ref $block))
...
(func $caml_lazy_reset_to_lazy (export "caml_lazy_reset_to_lazy")
  (param $v (ref eq)) (result (ref eq))
  (drop
    (call $obj_update_tag (local.get $v) (global.get $forcing_tag)
      (global.get $lazy_tag)))
  (ref.i31 (i32.const 0))
)
...
(func $caml_lazy_update_to_forcing (export "caml_lazy_update_to_forcing")
  ...
  (if (ref.test (ref $block) (local.get $v))
    (then
      (if
        (call $obj_update_tag (local.get $v) (global.get $lazy_tag)
...
(func $caml_obj_compare_and_swap (export "caml_obj_compare_and_swap")
  (param $v (ref eq)) (param $vi (ref eq)) (param $old (ref eq))
  ...
  (if (ref.test (ref $block) (local.get $o))
    (then
      (array.set $block (ref.cast (ref $block) (local.get $o))
        (i32.add (i31.get_u (ref.cast (ref i31) (local.get $i)))
          (i32.const 1)) (local.get $v))))
  (ref.i31 (i32.const 0))
)

(global $not_implemented (ref $bytes)
  (@string "Obj.add_offset is not supported" )
)
...
(global $truncate_not_implemented (ref $bytes)
  (@string "Obj.truncate is not supported" )
)
...
(func $caml_oo_cache_id (export "caml_oo_cache_id") (result (ref eq))
  (local $cacheid i32) (local $len i32) (local $a (ref $int_array))
  (local.set $cacheid (global.get $caml_oo_cache_id_last))
  (global.set $caml_oo_cache_id_last
    (i32.add (local.get $cacheid) (i32.const 1)))
...
(func $caml_get_cached_method (export "caml_get_cached_method")
  (param $obj (ref eq)) (param $vtag (ref eq)) (param $vcacheid (ref eq))
  (result (ref eq))
  (local $meths (ref $block)) (local $cacheid i32) (local $ofs i32)
  (local $tg i32) (local $li i32) (local $hi i32) (local $mi i32)
  (local.set $meths
    (ref.cast (ref $block)
...
(func $caml_get_public_method (export "caml_get_public_method")
  (param $obj (ref eq)) (param $vtag (ref eq)) (result (ref eq))
  (local $meths (ref $block)) (local $tg i32) (local $li i32) (local $hi i32)
  (local $mi i32)
  (local.set $meths
    (ref.cast (ref $block)
      (array.get $block (ref.cast (ref $block) (local.get $obj))
...
(func $caml_set_oo_id (export "caml_set_oo_id")
  ...
  (array.set $block (ref.cast (ref $block) (local.get $v)) (i32.const 2)
...
(func $caml_int_as_pointer (export "caml_int_as_pointer")
  (param $x (ref eq)) (result (ref eq))
  (if (i32.eqz (ref.eq (local.get $x) (ref.i31 (i32.const 0))))
    (then (call $caml_failwith (global.get $int_as_pointer_not_implemented))))
