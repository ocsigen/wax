(@if $wasi
(@then (import "libc" "memory" (memory $m 2))
...
(func $checked_malloc (export "checked_malloc") (param $size i32) (result i32)
  (local $p i32)
...
(func $write_string_to_memory (export "write_string_to_memory")
  ...
  (i32.store8 $m (i32.add (local.get $buf) (local.get $len)) (i32.const 0))
  (local.get $buf)
)
