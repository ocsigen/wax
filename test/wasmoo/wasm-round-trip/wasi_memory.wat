(@if $wasi
  (@then
    ...
    (func $checked_malloc (export "checked_malloc")
      ...
      (if (i32.eqz (local.get $p)) (then (call $caml_raise_out_of_memory)))
      (local.get $p))

    (func $blit_substring_to_memory (export "blit_substring_to_memory")
      (param $buf i32) (param $s (ref $bytes)) (param $ofs i32)
      (param $len i32)
      (local $i i32)
      (loop $loop
        (if (i32.lt_u (local.get $i) (local.get $len))
          (then
            (i32.store8 $m (i32.add (local.get $buf) (local.get $i))
    ...
    (func $blit_memory_to_substring (export "blit_memory_to_substring")
      (param $buf i32) (param $s (ref $bytes)) (param $ofs i32)
      (param $len i32)
      (local $i i32)
      (loop $loop
    ...
    (func $write_string_to_memory (export "write_string_to_memory")
      (param $buf i32) (param $avail i32) (param $v (ref eq)) (result i32)
      (local $i i32) (local $s (ref $bytes)) (local $len i32)
      (local.set $s (ref.cast (ref $bytes) (local.get $v)))
      (local.set $len (array.len (local.get $s)))
      (if
        (i32.lt_u (local.get $avail) (i32.add (local.get $len) (i32.const 1)))
        (then
          (local.set $buf
            (call $checked_malloc (i32.add (local.get $len) (i32.const 1))))))
      (call $blit_string_to_memory (local.get $buf) (local.get $s))
      (i32.store8 $m (i32.add (local.get $buf) (local.get $len))
        (i32.const 0))
      (local.get $buf))

    (func $release_memory (export "release_memory")
      (param $initial_buffer i32) (param $buf i32)
      (if (i32.ne (local.get $initial_buffer) (local.get $buf))
        (then (call $free (local.get $buf)))))
