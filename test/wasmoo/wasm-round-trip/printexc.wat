...
(func $add_char (param $buf (ref $buffer)) (param $c i32)
  (local $pos i32) (local $data_2 (ref $string))
  (local.set $pos (struct.get $buffer $f (local.get $buf)))
  (local.set $data_2 (struct.get $buffer $f_2 (local.get $buf)))
  (if (i32.lt_u (local.get $pos) (array.len (local.get $data_2)))
    (then
      (array.set $string (local.get $data_2) (local.get $pos) (local.get $c))
      (struct.set $buffer $f (local.get $buf)
        (i32.add (local.get $pos) (i32.const 1)))))
)

(func $add_string (param $buf (ref $buffer)) (param $v (ref eq))
  (local $pos i32) (local $data_2 (ref $string)) (local $s (ref $string))
  (local $len i32)
  (local.set $pos (struct.get $buffer $f (local.get $buf)))
  (local.set $data_2 (struct.get $buffer $f_2 (local.get $buf)))
  ...
  (if
    (i32.gt_u (i32.add (local.get $pos) (local.get $len))
      (array.len (local.get $data_2)))
    (then
      (local.set $len
        (i32.sub (array.len (local.get $data_2)) (local.get $pos)))))
  (array.copy $string $string (local.get $data_2) (local.get $pos)
