...
(func $ensure_capacity (param $buf (ref $buffer)) (param $extra i32)
  (local $pos i32) (local $dat (ref $bytes)) (local $cap i32)
  (local $need i32) (local $newcap i32) (local $new (ref $bytes))
  (local.set $pos (struct.get $buffer $pos (local.get $buf)))
  (local.set $dat (struct.get $buffer $dat (local.get $buf)))
  ...
  (if (i32.gt_u (local.get $need) (local.get $cap))
    (then
      ...
      (struct.set $buffer $dat (local.get $buf) (local.get $new))))
)
...
(func $add_char (param $buf (ref $buffer)) (param $c i32)
  ...
  (local.set $pos (struct.get $buffer $pos (local.get $buf)))
  (array.set $bytes (struct.get $buffer $dat (local.get $buf))
    (local.get $pos) (local.get $c))
  (struct.set $buffer $pos (local.get $buf)
    (i32.add (local.get $pos) (i32.const 1)))
)

(func $add_string (param $buf (ref $buffer)) (param $v (ref eq))
  (local $s (ref $bytes)) (local $len i32) (local $pos i32)
  (local.set $s (ref.cast (ref $bytes) (local.get $v)))
  (local.set $len (array.len (local.get $s)))
  (call $ensure_capacity (local.get $buf) (local.get $len))
  (local.set $pos (struct.get $buffer $pos (local.get $buf)))
  (array.copy $bytes $bytes (struct.get $buffer $dat (local.get $buf))
  ...
  (if (result (ref eq))
    ...
    (then
      ...
      (array.copy $bytes $bytes (local.get $s) (i32.const 0)
        (struct.get $buffer $dat (local.get $buf)) (i32.const 0)
        (struct.get $buffer $pos (local.get $buf)))
      (local.get $s))
