...
(@if (not $wasi)
  (@then
        ...
        (i32.ne (i32.const 0)
          (call $compare_strings
            (call $fromCharCodeArray
              (array.new_fixed $wstring 1 (i32.const 0)) (i32.const 0)
              (i32.const 1))
            (call $fromCharCodeArray
              (array.new_fixed $wstring 1 (i32.const 1)) (i32.const 0)
              (i32.const 1)))))
          ...
          (return
            (any.convert_extern
              (call $fromCharCodeArray (global.get $buffer) (i32.const 0)
                (local.get $len))))))
      (return_call $jsstring_of_subbytes_fallback (local.get $s)
    ...
    (func $string_of_jsstring_fallback (param $s anyref) (result (ref $bytes))
      ...
      (call $read_from_buffer (local.get $s') (local.get $ofs)
        (local.get $len))
      (block $done
        (local.set $item (br_on_null $done (global.get $stack)))
    ...
    (func $caml_extract_bytes (export "caml_extract_bytes") (param $len i32)
      ...
      (global.set $stack
        (struct.new $stack (local.get $s) (global.get $stack)))))
)
