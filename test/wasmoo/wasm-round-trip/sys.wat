...
(@if $wasi
  ...
  (@else
    ...
    (import "bindings" "random_seed"
      (func $random_seed (result (ref extern))))
                ...
                (loop $loop2
                  (if (i32.lt_u (local.get $j) (local.get $len))
                    (then
                      ...
                      (br_if $next
                        (i32.ne (local.get $c)
                          (array.get_u $bytes (local.get $var)
                            (local.get $j))))
...
(@if $wasi
  ...
  (@else
    ...
    (func $caml_sys_executable_name (export "caml_sys_executable_name")
      ...
      (array.get $block
        (ref.cast (ref $block)
          (call $caml_js_to_string_array (call $argv_fn))) (i32.const 1))))
)
      ...
      (try
        (do
          (return
            (call $system
              (call $unwrap
                (call $caml_jsstring_of_string (local.get $cmd))))))
...
(@if $wasi
  (@then
    (func $caml_sys_random_seed (export "caml_sys_random_seed")
      (param (ref eq)) (result (ref eq))
      (local $r (ref extern)) (local $n i32) (local $buffer i32)
      (local $res i32) (local $a (ref $block)) (local $i i32)
...
(@if $wasi
  (@then
    ...
    (func $caml_handle_sys_error_if (export "caml_handle_sys_error_if")
      ...
      (if (local.get $errno)
        (then
          (call $caml_handle_sys_error (local.get $arg)
            (local.get $errno))))))
  (@else
    (func $caml_handle_sys_error (export "caml_handle_sys_error")
      (param $exn externref)
