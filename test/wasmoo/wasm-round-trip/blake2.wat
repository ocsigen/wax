...
(@then (import "blake2" "blake2_memory" (memory $m 1))
...
(import "blake2" "blake2_chunk_buf_size" (func $chunk_buf_size (result i32)))
(import "blake2" "blake2_init" (func $blake2_init (param i32 i32)))
(import "blake2" "blake2_update" (func $blake2_update (param i32)))
(import "blake2" "blake2_finalize" (func $blake2_finalize (param i32)))
...
(func $bytes_to_mem
  (param $src (ref $bytes)) (param $src_ofs i32) (param $dst i32)
  (param $len i32)
  (local $i i32)
  (block $done
    (loop $cont
...
(func $do_create
  ...
  (call $bytes_to_mem (local.get $key) (i32.const 0) (call $key_buf)
    (local.get $keylen))
  (call $blake2_init (local.get $hashlen) (local.get $keylen))
  (call $mem_to_bytes (call $state_buf) (local.get $state) (i32.const 0)
...
(func $do_update
  (param $state (ref $bytes)) (param $buf (ref $bytes)) (param $ofs i32)
  (param $len i32)
  (local $chunk_max i32) (local $chunk i32)
  (call $bytes_to_mem (local.get $state) (i32.const 0) (call $state_buf)
    (global.get $blake2_state_size))
  ...
  (block $done
    (loop $cont
      ...
      (local.set $chunk
        (select (local.get $chunk_max) (local.get $len)
          (i32.gt_u (local.get $len) (local.get $chunk_max))))
      (call $bytes_to_mem (local.get $buf) (local.get $ofs) (call $chunk_buf)
        (local.get $chunk))
...
(func $do_final
  ...
  (local.set $output (array.new $bytes (i32.const 0) (local.get $hashlen)))
  (call $mem_to_bytes (call $out_buf) (local.get $output) (i32.const 0)
    (local.get $hashlen))
  (local.get $output)
)

(func $caml_blake2_create (export "caml_blake2_create")
  (param $hashlen (ref eq)) (param $key (ref eq)) (result (ref eq))
  (call $do_create (i31.get_u (ref.cast (ref i31) (local.get $hashlen)))
    (ref.cast (ref $bytes) (local.get $key)))
)

(func $caml_blake2_update (export "caml_blake2_update")
  (param $ctx (ref eq)) (param $buf (ref eq)) (param $ofs (ref eq))
  (param $len (ref eq)) (result (ref eq))
  (call $do_update (ref.cast (ref $bytes) (local.get $ctx))
...
(func $caml_blake2_final (export "caml_blake2_final")
  (param $ctx (ref eq)) (param $hashlen (ref eq)) (result (ref eq))
  (call $do_final (ref.cast (ref $bytes) (local.get $ctx))
    (i31.get_u (ref.cast (ref i31) (local.get $hashlen))))
)

(func $caml_blake2_string (export "caml_blake2_string")
  ...
  (local $hl i32) (local $state (ref $bytes))
  (local.set $hl (i31.get_u (ref.cast (ref i31) (local.get $hashlen))))
  (local.set $state
    (call $do_create (local.get $hl)
      (ref.cast (ref $bytes) (local.get $key))))
  (call $do_update (local.get $state) (ref.cast (ref $bytes) (local.get $buf))
    (i31.get_u (ref.cast (ref i31) (local.get $ofs)))
    (i31.get_u (ref.cast (ref i31) (local.get $len))))
