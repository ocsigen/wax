...
(global $lexing_empty_token (ref $bytes) (@string "lexing: empty token"))

(func $caml_lex_engine (export "caml_lex_engine")
  (param $vtbl (ref eq)) (param $start_state (ref eq))
  (param $vlexbuf (ref eq)) (result (ref eq))
  (local $tbl (ref $block)) (local $lexbuf (ref $block)) (local $state i32)
  (local $buffer (ref $bytes)) (local $vpos (ref eq))
  (local $lex_base (ref $bytes)) (local $lex_backtrk (ref $bytes))
  (local $lex_check (ref $bytes)) (local $lex_trans (ref $bytes))
  (local $lex_default (ref $bytes)) (local $c i32) (local $base i32)
  (local $backtrk i32) (local $pos i32) (local $action (ref eq))
...
(func $run_mem
  ...
  (local $mem (ref $block)) (local $dst i32) (local $src i32)
  (local.set $mem
    (ref.cast (ref $block)
...
(func $caml_new_lex_engine (export "caml_new_lex_engine")
  ...
  (local $tbl (ref $block)) (local $lexbuf (ref $block)) (local $state i32)
  (local $buffer (ref $bytes)) (local $vpos (ref eq))
  (local $lex_code (ref $bytes)) (local $lex_base (ref $bytes))
  (local $lex_base_code (ref $bytes)) (local $lex_backtrk (ref $bytes))
  (local $lex_backtrk_code (ref $bytes)) (local $lex_check (ref $bytes))
  (local $lex_check_code (ref $bytes)) (local $lex_trans (ref $bytes))
  (local $lex_trans_code (ref $bytes)) (local $lex_default (ref $bytes))
  (local $lex_default_code (ref $bytes)) (local $c i32) (local $base i32)
  (local $pc_off i32) (local $backtrk i32) (local $pos i32)
  (local $pstate i32) (local $action (ref eq)) (local $base_code i32)
  (local.set $tbl (ref.cast (ref $block) (local.get $vtbl)))
  (local.set $lexbuf (ref.cast (ref $block) (local.get $vlexbuf)))
  (local.set $state (i31.get_s (ref.cast (ref i31) (local.get $start_state))))
  (local.set $buffer
