...
(@if $wasi
  (@then
    ...
    (func $ta_copy
      (param $buf (ref extern)) (param $dst i32) (param $src i32)
      (param $end i32)
      (local $b (ref $bytes))
      (local.set $b
...
(@if $wasi
  (@then
    ...
    (global $sys_open_flags (ref $open_flags)
      (array.new_fixed $open_flags 9 (i32.const 1) (i32.const 2)
        (i32.const 0x102) (i32.const 0x10) (i32.const 0x80) (i32.const 0x40)
        (i32.const 0) (i32.const 0) (i32.const 0x400))))
  (@else
    ;;   1 O_RDONLY
    ;;   2 O_WRONLY
    ;;   4 O_RDWR
    ...
    (global $sys_open_flags (ref $open_flags)
      (array.new_fixed $open_flags 9 (i32.const 1) (i32.const 2)
        (i32.const 10) (i32.const 16) (i32.const 32) (i32.const 64)
        (i32.const 0) (i32.const 0) (i32.const 128))))
)
...
(@if $wasi
  (@then
    (func $push_channel
      (param $l (ref eq)) (param $ch (ref eq)) (result (ref eq))
      (local $c (ref $channel))
      (block $continue
...
(@if $wasi
  (@then
    (func $read
      (param $fd i32) (param $buf (ref extern)) (param $pos i32)
...
(@if $wasi
  (@then
    ...
    (func $caml_do_read_or_refill
      (param $ch (ref $channel)) (param $pos i32) (param $len i32)
      (result i32)
  ...
  (@else
    (func $caml_do_read_or_refill
      ...
      (local $size i32) (local $buffer (ref extern))
...
(func $caml_getblock (export "caml_getblock")
  ...
  (param $curr i32) (result i32)
  (local $ch (ref $channel)) (local $avail i32) (local $max i32)
  (if (i32.eqz (local.get $curr)) (then (return (i32.const 0))))
  (local.set $ch (ref.cast (ref $channel) (local.get $vch)))
  (local.set $avail
  ...
  (call $caml_blit_dataview_to_bytes
    ...
    (local.get $s) (local.get $pos) (local.get $curr))
...
(func $caml_getblock_typed_array
  ...
  (if (i32.gt_u (local.get $curr) (local.get $max))
    (then (local.set $curr (local.get $max))))
  (call $ta_blit_from_buffer (struct.get $channel $buffer (local.get $ch))
    (i32.const 0) (local.get $d) (local.get $pos) (local.get $curr))
  (struct.set $channel $curr (local.get $ch) (local.get $curr))
  (local.get $curr)
)
...
(@if $wasi
  (@then
    (func $write_all_to_fd (export "write_all_to_fd")
      ...
      (local $iovs i32) (local $buf i32) (local $remaining i32)
      (local $res i32)
    ...
    (func $caml_flush_partial (param $ch (ref $channel)) (result i32)
      ...
      (local $tmp (ref $bytes)) (local $curr i32) (local $buf (ref extern))
          ...
          (if (i32.gt_u (local.get $curr) (local.get $written))
            (then
              (call $ta_copy (local.get $buf) (i32.const 0)
                (local.get $written) (local.get $curr))))
          (local.set $curr (i32.sub (local.get $curr) (local.get $written)))
          (struct.set $channel $curr (local.get $ch) (local.get $curr))))
      (i32.eqz (local.get $curr))))
  (@else
    (func $caml_flush_partial (param $ch (ref $channel)) (result i32)
      (local $curr i32) (local $buf (ref extern)) (local $fd i32)
      (local $fd_offset (ref $fd_offset)) (local $offset i64)
      (local $written i32) (local $tmp (ref $bytes))
      (local.set $curr (struct.get $channel $curr (local.get $ch)))
      (if (i32.gt_u (local.get $curr) (i32.const 0))
        (then
          (local.set $buf (struct.get $channel $buffer (local.get $ch)))
          (if (ref.is_null (struct.get $channel $output (local.get $ch)))
            ...
            (else
              ...
              (call $caml_blit_dataview_to_bytes
                (struct.get $channel $buffer_view (local.get $ch))
                (i32.const 0) (local.get $tmp) (i32.const 0)
                (local.get $curr))
              (drop
