...
(import "wasi_memory" "release_memory" (func $release_memory (param i32 i32)))
...
(import "jslib" "caml_js_fun_call"
  (func $caml_js_fun_call (param (ref eq) (ref eq)) (result (ref eq)))
)
...
(func $ta_copy
  (param $buf (ref extern)) (param $dst i32) (param $src i32) (param $end i32)
  (local $b (ref $bytes))
  (local.set $b (ref.cast (ref $bytes) (any.convert_extern (local.get $buf))))
  (array.copy $bytes $bytes (local.get $b) (local.get $dst) (local.get $b)
    (local.get $src) (i32.sub (local.get $end) (local.get $src)))
)
...
(func $caml_blit_bytes_to_dataview
  ...
  (array.copy $bytes $bytes
    ...
    (local.get $j) (local.get $s) (local.get $i) (local.get $l))
)

(func $caml_blit_dataview_to_bytes
  (param $buf (ref extern)) (param $i i32) (param $s (ref $bytes))
  (param $j i32) (param $l i32)
  (array.copy $bytes $bytes (local.get $s) (local.get $j)
    (ref.cast (ref $bytes) (any.convert_extern (local.get $buf)))
    (local.get $i) (local.get $l))
)

(func $dv_make (param $b (ref extern)) (result (ref extern)) (local.get $b))

(func $dv_get_ui8 (param $a (ref extern)) (param $i i32) (result i32)
  (array.get_u $bytes
    (ref.cast (ref $bytes) (any.convert_extern (local.get $a)))
    (local.get $i))
)

(func $dv_set_i8 (param $a (ref extern)) (param $i i32) (param $v i32)
  (array.set $bytes
    (ref.cast (ref $bytes) (any.convert_extern (local.get $a))) (local.get $i)
    (local.get $v))
)
...
(func $ta_blit_from_buffer
  (param $buf (ref extern)) (param $i i32) (param $ta (ref extern))
  (param $j i32) (param $len i32)
  (local $dat (ref $dat))
  (local.set $dat (ref.cast (ref $dat) (any.convert_extern (local.get $ta))))
  (call $caml_blit_dataview_to_bytes (local.get $buf) (local.get $i)
...
(func $ta_blit_to_buffer
  (param $ta (ref extern)) (param $i i32) (param $buf (ref extern))
  (param $j i32) (param $len i32)
  (local $dat (ref $dat))
  (local.set $dat (ref.cast (ref $dat) (any.convert_extern (local.get $ta))))
  (call $caml_blit_bytes_to_dataview
    ...
    (local.get $buf) (local.get $j) (local.get $len))
)

(global $caml_stdout (mut (ref eq)) (ref.i31 (i32.const 0)))
...
(func $register_channel (param $ch (ref eq))
  (if
    (i32.eq
      ...
      (i32.const 1))
    (then (global.set $caml_stdout (local.get $ch))))
  (global.set $out_channels
    (array.new_fixed $block 3 (ref.i31 (i32.const 0)) (local.get $ch)
...
(func $file_size (param $fd i32) (result i64)
  (local $buffer i32) (local $res i32) (local $cur i64) (local $end i64)
  (local.set $buffer (call $get_buffer))
  (block $error
    (local.set $res
      (call $fd_seek (local.get $fd) (i64.const 0) (i32.const 1)
        (local.get $buffer)))
    (br_if $error (local.get $res))
    (local.set $cur (i64.load $m (local.get $buffer)))
    (local.set $res
      (call $fd_seek (local.get $fd) (i64.const 0) (i32.const 2)
        (local.get $buffer)))
    (br_if $error (local.get $res))
    (local.set $end (i64.load $m (local.get $buffer)))
    (local.set $res
      (call $fd_seek (local.get $fd) (local.get $cur) (i32.const 0)
        (local.get $buffer)))
    (br_if $error (local.get $res))
...
(@else (import "bindings" "map_new" (func $map_new (result (ref extern))))
(import "bindings" "map_get"
  (func $map_get (param (ref extern) i32) (result (ref null $fd_offset)))
)
...
(func $ta_blit_from_buffer
  (param $buf (ref extern)) (param $i i32) (param $ta (ref extern))
  (param $j i32) (param $len i32)
  (call $ta_set (local.get $ta)
    (call $ta_subarray (local.get $buf) (local.get $i)
      (i32.add (local.get $i) (local.get $len))) (local.get $j))
)
...
(global $channel_ops (ref $custom_operations)
  (struct.new $custom_operations (@string "_chan" )
...
(type $channel
  (sub final $custom_with_id
    (struct
      ...
      (field $output (mut eqref))
      (field $refill (mut eqref))))
)
...
(global $bad_file_descriptor (ref $bytes) (@string "Bad file descriptor" ))
...
(global $sys_open_flags (ref $open_flags)
  (array.new_fixed $open_flags 9 (i32.const 1) (i32.const 2) (i32.const 0x102)
    (i32.const 0x10) (i32.const 0x80) (i32.const 0x40) (i32.const 0)
    (i32.const 0) (i32.const 0x400))
) )
(@else
;;   1 O_RDONLY
;;   2 O_WRONLY
...
;; 128 O_NONBLOCK
(global $sys_open_flags (ref $open_flags)
  (array.new_fixed $open_flags 9 (i32.const 1) (i32.const 2) (i32.const 10)
    (i32.const 16) (i32.const 32) (i32.const 64) (i32.const 0) (i32.const 0)
    (i32.const 128))
) ) )
...
(func $caml_sys_open (export "caml_sys_open")
  ...
  (result (ref eq))
  (local $path_2 i32) (local $path_1 i32) (local $path_0 i32)
  (local $buffer i32) (local $flags i32) (local $res i32) (local $fd i32)
  (local $offset i64)
  ...
  (local.set $buffer (call $get_buffer))
  (local.set $flags
    (call $convert_flag_list (global.get $sys_open_flags)
      (local.get $vflags)))
  (local.set $res
    (call $path_open (local.get $path_0) (i32.const 1) ;; symlink_follow
      (local.get $path_1) (local.get $path_2)
      (i32.and (i32.shr_u (local.get $flags) (i32.const 4)) (i32.const 0xF))
      (select (i64.const 0x860007c) (i64.const 0x820003e)
        (i32.and (local.get $flags) (i32.const 2))) (i64.const 0)
      (i32.shr_u (local.get $flags) (i32.const 8)) (local.get $buffer)))
...
(func $caml_sys_close (export "caml_sys_close")
  (param $vfd (ref eq)) (result (ref eq))
...
(func $caml_sys_io_buffer_size (export "caml_sys_io_buffer_size")
...
;; nothing.
(func $caml_ml_set_channel_output (export "caml_ml_set_channel_output")
...
(func $caml_ml_set_channel_refill (export "caml_ml_set_channel_refill")
  (param $ch (ref eq)) (param $f (ref eq)) (result (ref eq))
  (call $caml_invalid_argument
...
(func $caml_ml_set_channel_output (export "caml_ml_set_channel_output")
  (param $ch (ref eq)) (param $f (ref eq)) (result (ref eq))
  (struct.set $channel $output (ref.cast (ref $channel) (local.get $ch))
    (local.get $f))
  (ref.i31 (i32.const 0))
)
...
(func $caml_ml_set_channel_refill (export "caml_ml_set_channel_refill")
  (param $ch (ref eq)) (param $f (ref eq)) (result (ref eq))
  (struct.set $channel $refill (ref.cast (ref $channel) (local.get $ch))
    (local.get $f))
  (ref.i31 (i32.const 0))
) ) )
...
(func $caml_ml_open_descriptor_in (export "caml_ml_open_descriptor_in")
  ...
  (struct.new $channel (global.get $channel_ops) (call $custom_next_id)
    (i31.get_u (ref.cast (ref i31) (local.get $fd))) (local.get $buffer)
    (call $dv_make (local.get $buffer)) (i32.const 0) (i32.const 0)
    (global.get $IO_BUFFER_SIZE) (i32.const 0) (ref.null eq) (ref.null eq))
)
...
(func $caml_ml_open_descriptor_out (export "caml_ml_open_descriptor_out")
  ...
  (local.set $res
    (struct.new $channel (global.get $channel_ops) (call $custom_next_id)
      (i31.get_u (ref.cast (ref i31) (local.get $fd))) (local.get $buffer)
      (call $dv_make (local.get $buffer)) (i32.const 0) (i32.const -1)
      (global.get $IO_BUFFER_SIZE) (i32.const 0) (ref.null eq) (ref.null eq)))
...
(func $caml_ml_set_binary_mode (export "caml_ml_set_binary_mode")
  (param (ref eq) (ref eq)) (result (ref eq))
  (ref.i31 (i32.const 0))
)

(func $caml_ml_is_binary_mode (export "caml_ml_is_binary_mode")
  (param (ref eq)) (result (ref eq))
  (ref.i31 (i32.const 1))
)

(func $caml_channel_descriptor (export "caml_channel_descriptor")
...
(func $caml_ml_close_channel (export "caml_ml_close_channel")
  ...
  (local $ch (ref $channel)) (local $fd i32) (local $res i32)
...
(func $read
  ...
  (local $iovs_len i32) (local $res i32) (local $s (ref $bytes))
  (local.set $buffer (call $get_buffer))
  ...
  (i32.store $m offset=4 (local.get $iovs) (local.get $n))
  ...
  (local.set $s (ref.cast (ref $bytes) (any.convert_extern (local.get $buf))))
  (call $blit_memory_to_substring (local.get $buffer) (local.get $s)
...
(func $caml_do_read_or_refill
  (param $ch (ref $channel)) (param $pos i32) (param $len i32) (result i32)
  (return_call $caml_do_read (local.get $ch) (local.get $pos)
    (local.get $len))
) )
(@else
(func $caml_do_read_or_refill
  (param $ch (ref $channel)) (param $pos i32) (param $len i32) (result i32)
  (local $f eqref) (local $str (ref $bytes)) (local $str_len i32)
  (local $new_size i32) (local $new_buffer (ref extern))
  (local $fd_offset (ref $fd_offset))
  (local.set $f (struct.get $channel $refill (local.get $ch)))
  (if (ref.is_null (local.get $f))
    (then
      (return
        (call $caml_do_read (local.get $ch) (local.get $pos)
          (local.get $len)))))
  (local.set $str
    (ref.cast (ref $bytes)
      (call $caml_js_fun_call (ref.as_non_null (local.get $f))
  ...
  (if
    (i32.gt_u (i32.add (local.get $pos) (local.get $str_len))
  ...
  (call $caml_blit_bytes_to_dataview (local.get $str) (i32.const 0)
...
(func $caml_refill (param $ch (ref $channel)) (result i32)
  (local $view (ref extern)) (local $n i32)
  (local.set $view (struct.get $channel $buffer_view (local.get $ch)))
  (local.set $n
    (call $caml_do_read_or_refill (local.get $ch) (i32.const 0)
      (struct.get $channel $size (local.get $ch))))
...
(func $caml_getblock (export "caml_getblock")
  (param $vch (ref eq)) (param $s (ref $bytes)) (param $pos i32)
  ...
  (call $caml_blit_dataview_to_bytes
    (struct.get $channel $buffer_view (local.get $ch)) (i32.const 0)
...
(func $caml_getblock_typed_array
  (param $vch (ref eq)) (param $d (ref extern)) (param $pos i32)
  (param $len i32) (result i32)
  (local $ch (ref $channel)) (local $avail i32) (local $nread i32)
  (if (i32.eqz (local.get $len)) (then (return (i32.const 0))))
  (local.set $ch (ref.cast (ref $channel) (local.get $vch)))
  ...
  (if (local.get $avail)
    (then
      (if (i32.gt_u (local.get $len) (local.get $avail))
        (then (local.set $len (local.get $avail))))
      (call $ta_blit_from_buffer (struct.get $channel $buffer (local.get $ch))
        (struct.get $channel $curr (local.get $ch)) (local.get $d)
        (local.get $pos) (local.get $len))
      (struct.set $channel $curr (local.get $ch)
        (i32.add (struct.get $channel $curr (local.get $ch))
          (local.get $len)))
  ...
  (if (i32.gt_u (local.get $len) (local.get $nread))
    (then (local.set $len (local.get $nread))))
  (call $ta_blit_from_buffer (struct.get $channel $buffer (local.get $ch))
    (i32.const 0) (local.get $d) (local.get $pos) (local.get $len))
  (struct.set $channel $curr (local.get $ch) (local.get $len))
  (local.get $len)
)

(func $caml_really_getblock (export "caml_really_getblock")
  (param $ch (ref eq)) (param $s (ref $bytes)) (param $pos i32)
  (param $len i32) (result i32)
...
(func $caml_ml_input (export "caml_ml_input")
  ...
  (local $ch (ref $channel)) (local $s (ref $bytes)) (local $pos i32)
  (local $len i32) (local $buf (ref extern)) (local $curr i32)
  ...
  (local.set $buf (struct.get $channel $buffer_view (local.get $ch)))
  (call $caml_blit_dataview_to_bytes (local.get $buf) (local.get $curr)
    (local.get $s) (local.get $pos) (local.get $len))
  (struct.set $channel $curr (local.get $ch)
...
(func $caml_getch (param $ch (ref $channel)) (result i32)
  ...
  (return_call $dv_get_ui8 (struct.get $channel $buffer_view (local.get $ch))
    (local.get $curr))
)
...
(func $caml_ml_pos_in (export "caml_ml_pos_in")
  ...
  (local.set $ch (ref.cast (ref $channel) (local.get $vch)))
  (ref.i31
    (i32.sub (i32.wrap_i64 (call $caml_ml_get_channel_offset (local.get $ch)))
      (i32.sub (struct.get $channel $max (local.get $ch))
        (struct.get $channel $curr (local.get $ch)))))
)
...
(func $caml_ml_pos_out (export "caml_ml_pos_out")
  ...
  (local.set $ch (ref.cast (ref $channel) (local.get $vch)))
  (ref.i31
    (i32.add (i32.wrap_i64 (call $caml_ml_get_channel_offset (local.get $ch)))
      (struct.get $channel $curr (local.get $ch))))
)
...
(func $caml_seek_in
  ...
  (local.set $res
    (call $fd_seek (local.get $fd) (local.get $dest) (i32.const 0)
      (local.get $buffer)))
  (call $caml_handle_sys_error_if (ref.i31 (i32.const 0)) (local.get $res))
...
(func $caml_ml_seek_out (export "caml_ml_seek_out")
  ...
  (local $offset i64) (local $buffer i32) (local $res i32)
  (local.set $ch (ref.cast (ref $channel) (local.get $vch)))
...
(func $caml_ml_input_scan_line (export "caml_ml_input_scan_line")
  ...
  (loop $loop
    ...
    (if
      (i32.eq (i32.const 10) ;; '\n'
        (call $dv_get_ui8 (struct.get $channel $buffer_view (local.get $ch))
          (local.get $p)))
      (then
        (return
          (ref.i31
...
(func $write
  ...
  (local $iovs_len i32) (local $s (ref $bytes)) (local $res i32)
  (local.set $buffer (call $get_buffer))
...
(func $write_all_to_fd (export "write_all_to_fd")
  ...
  (local $len i32) (local $buffer i32) (local $nwritten i32) (local $iovs i32)
  (local $buf i32) (local $remaining i32) (local $res i32)
  ...
  (local.set $buffer (i32.add (local.get $buffer) (i32.const 12)))
  (local.set $buf
    (call $write_string_to_memory (local.get $buffer)
      (global.get $IO_BUFFER_SIZE) (local.get $v)))
  (local.set $remaining (local.get $buf))
  (loop $write
    (i32.store $m (local.get $iovs) (local.get $remaining))
    (i32.store $m offset=4 (local.get $iovs) (local.get $len))
    (local.set $res
      (call $fd_write (local.get $fd) (local.get $iovs) (i32.const 1)
        (local.get $nwritten)))
    (if (i32.eqz (local.get $res))
      (then
        (local.set $len
          (i32.sub (local.get $len) (i32.load $m (local.get $nwritten))))
        (local.set $remaining
...
(func $caml_flush_partial (param $ch (ref $channel)) (result i32)
  ...
  (local $tmp (ref $bytes)) (local $towrite i32) (local $buf (ref extern))
  (local $fd i32) (local $written i32)
  ...
  (if (i32.gt_u (local.get $towrite) (i32.const 0))
    (then
      ...
      (if (ref.is_null (struct.get $channel $output (local.get $ch)))
                ...
                (if (result i32)
                  ...
                  (then
                    (call $write (local.get $fd) (local.get $buf)
                      (i32.const 0) (local.get $towrite) (local.get $offset)))
                  (else
                    (call $write' (local.get $fd) (local.get $buf)
                      (i32.const 0) (local.get $towrite)
                      (ref.null noextern))))))
            (catch $javascript_exception (call $caml_handle_sys_error)))
          (struct.set $fd_offset $offset (local.get $fd_offset)
        ...
        (else
          (local.set $tmp
            (array.new $bytes (i32.const 0) (local.get $towrite)))
          (call $caml_blit_dataview_to_bytes
            (struct.get $channel $buffer_view (local.get $ch)) (i32.const 0)
            (local.get $tmp) (i32.const 0) (local.get $towrite))
          (drop
            (call $caml_js_fun_call
              (ref.as_non_null (struct.get $channel $output (local.get $ch)))
              (array.new_fixed $block 2 (ref.i31 (i32.const 0))
                (call $caml_jsbytes_of_string (local.get $tmp)))))
          (struct.set $channel $curr (local.get $ch) (i32.const 0))
          (local.set $towrite (i32.const 0))))))
  (i32.eqz (local.get $towrite))
) ) )
...
(func $caml_putblock_typed_array
  ...
  (if (i32.ge_u (local.get $len) (local.get $free))
    (then (local.set $len (local.get $free))))
  (local.set $buf (struct.get $channel $buffer (local.get $ch)))
  (call $ta_blit_to_buffer (local.get $d) (local.get $pos) (local.get $buf)
...
(func $caml_really_putblock_typed_array
  (param $ch (ref eq)) (param $d (ref extern)) (param $pos i32)
  (param $len i32)
  (local $written i32)
  (loop $loop
    (if (local.get $len)
      (then
        (local.set $written
          (call $caml_putblock_typed_array
            (ref.cast (ref $channel) (local.get $ch)) (local.get $d)
            (local.get $pos) (local.get $len)))
        (local.set $pos (i32.add (local.get $pos) (local.get $written)))
        (local.set $len (i32.sub (local.get $len) (local.get $written)))
        (br $loop))))
  ...
  (loop $loop
    (if (i32.gt_u (local.get $len) (i32.const 0))
      (then
        (local.set $written
          (call $caml_putblock (ref.cast (ref $channel) (local.get $ch))
            (ref.cast (ref $bytes) (local.get $s)) (local.get $pos)
            (local.get $len)))
        (local.set $pos (i32.add (local.get $pos) (local.get $written)))
        (local.set $len (i32.sub (local.get $len) (local.get $written)))
        (br $loop))))
...
(func $caml_ml_set_channel_fd (export "caml_ml_set_channel_fd")
  (param $vch (ref eq)) (param $fd i32)
  (struct.set $channel $fd (ref.cast (ref $channel) (local.get $vch))
    (local.get $fd))
)
...
(func $caml_ml_output_bigarray (export "caml_ml_output_bigarray")
  ...
  (local.set $pos (i31.get_u (ref.cast (ref i31) (local.get $vpos))))
  (local.set $len (i31.get_u (ref.cast (ref i31) (local.get $vlen))))
  (call $caml_really_putblock_typed_array (local.get $ch) (local.get $d)
    (local.get $pos) (local.get $len))
  (ref.i31 (i32.const 0))
)

(func $caml_ml_input_bigarray (export "caml_ml_input_bigarray")
  (param $ch (ref eq)) (param $a (ref eq)) (param $vpos (ref eq))
  (param $vlen (ref eq)) (result (ref eq))
  (local $d (ref extern)) (local $pos i32) (local $len i32)
  (local.set $d (call $caml_ba_get_data (local.get $a)))
  ...
  (local.set $len (i31.get_u (ref.cast (ref i31) (local.get $vlen))))
  (ref.i31
    (call $caml_getblock_typed_array (local.get $ch) (local.get $d)
      (local.get $pos) (local.get $len)))
)
