...
(global $lock_failure (ref $bytes)
  (@string "Mutex.lock: mutex already locked. Cannot wait.")
)
...
(func $caml_ml_mutex_unlock (export "caml_ml_mutex_unlock")
  (param $vt (ref eq)) (result (ref eq))
  (struct.set $mutex $state (ref.cast (ref $mutex) (local.get $vt))
    (i32.const 0))
  (ref.i31 (i32.const 0))
)
...
(type $condition
  (sub final $custom_with_id
    (struct (field $ops (ref $custom_operations)) (field $id i64)))
)
...
(func $caml_ml_condition_new (export "caml_ml_condition_new")
  ...
  (struct.new $condition (global.get $condition_ops) (call $custom_next_id))
)
